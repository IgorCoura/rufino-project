...
| **2.7** ✅ | **Concluída.** **`BillExpectation` + lembretes**: Aggregate, ciclos, `ExpectationMatchingService`, `ExpectationLearningService`, job diário de vigilância, escalonamento, `INotificationSender` por e-mail, painel de pendências ([`11-bill-expectations.md`](11-bill-expectations.md)). Medido: **10 dos 20 grupos de beneficiário têm mais de uma conta do mesmo tenant**, então a referência de conta entra na chave e é **informada**, não deduzida; o aprendizado recusa quando não sabe separar. `INotificationSender` entregue com adapter de log — e-mail é item do checklist |
...
**Entrega:** o boleto aprovado é pago na data. **Plano revisado em 2026-09-02** — a 3.0 original (subcontas criadas pela plataforma) foi substituída pelo modelo "conta trazida pelo tenant", já entregue em 2026-08-31 ([`adr/ADR-016`](adr/ADR-016-conta-asaas-trazida-pelo-tenant.md)); a política de agendamento está no [`adr/ADR-017`](adr/ADR-017-politica-inicial-de-agendamento.md), **revisto pelo [`adr/ADR-021`](adr/ADR-021-quatro-opcoes-de-agendamento.md)** (as 24h saíram, a janela virou 9h–18h e só limita "pagar hoje").
...
| **3.1** | `PaymentOrder` (PMO); portas `IBillPaymentGateway` + `IPixPaymentGateway` com **falha modelada** (molde `LookupResult`); handler de `BillApprovedDomainEvent` cria a ordem **sem chamada externa na transação**; **worker de submissão** serial (fila por coluna: reivindicação atômica, aluguel = backoff, teto, desistência visível; só submete na janela 9h–18h — 17h até o ADR-021); idempotência por consulta ao `externalReference` antes de retentar; cliente HTTP próprio **sem retry**; `Bill.LinkPaymentOrder` por evento; `BillCancelled` cancela ordem cancelável |
| **3.2** | `IWorkingDayCalendar` (feriados bancários como snapshot embutido, doutrina `IBankDirectory`) + `PaymentSchedulingService` (dia útil, corte das 14h, `minimumScheduleDate`, vencido = imediato, **mais a política do ADR-017**: 24h de antecedência, janela 9h–17h, deslize para o dia útil seguinte — **as 24h foram removidas e a janela virou 9h–18h em 2026-09-08, ADR-021**); **confirmação explícita para execução imediata/vencido** (na aprovação e na fila); ~~verificação de saldo com a chave do tenant e alerta pelo canal da 2.7~~ — **removida do escopo em 2026-09-03** (saldo nunca bloqueia agendamento; ver ADR-017, "Decisões posteriores") |
...
| **3.4** | Cancelamento de ordem (`payment:cancel`); tratamento de `FAILED`/`REFUNDED`; reabertura para nova tentativa; fila operacional (`GET /payments?status=Failed` + `lastError`) e alertas; recurso `payment` no realm; endpoints de custo na policy `expensive`; roteiro de replay da dead-letter do outbox (**escrito em 2026-09-03** — [`13-dead-letter-replay.md`](13-dead-letter-replay.md)) |
