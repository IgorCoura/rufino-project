...
| [`08-boleto-corpus-findings.md`](08-boleto-corpus-findings.md) | Medição de 39 boletos reais: taxa de extração, OCR obrigatório, arrecadação como 45% do volume, ambiguidade do fator de vencimento, pagador presente em só 38% |
...
| [`tools/`](tools/) | `analyze-boleto-corpus.js` (mede o corpus), `probe-asaas-simulate.js` (cobertura da consulta), `fetch-bacen-participants.js` (atualiza a tabela de bancos do Bacen), `smoke-probe-production.js` e `smoke-probe-pix-decode.js` (sondas da consulta oficial), `smoke-probe-mailbox.js` (sonda de leitura de caixa no Graph — read-only, mede os filtros de anexo contra uma caixa real) |
