...
| [`ADR-017`](adr/ADR-017-politica-inicial-de-agendamento.md) | Política inicial de agendamento — **parcialmente superado pelo ADR-021**; sobrevive a regra de que boleto vencido exige confirmação explícita gravada na trilha |
...
| [`ADR-025`](adr/ADR-025-pagador-verificavel-no-trilho-pix.md) | O pagador **é** verificável no Pix dinâmico com cobrança registrada — `PayerMatch` sai `Passed` com motivo próprio (`payer_confirmed_by_lookup`), com o escopo (QR dinâmico + documento inteiro + DV válido) preso no tipo; **limita o ADR-004**, que se declarava permanente |
| [`ADR-026`](adr/ADR-026-numero-da-conta-na-escada-de-roteamento.md) | O **número da conta** cadastrado na expectativa roteia a conta de concessionária (degrau 3, por conter, nunca por posição), sem travessia de tenant; e a reivindicação oferece **lembrar desta conta** |
| [`ADR-024`](adr/ADR-024-fonte-oficial-primeiro-e-a-regua-por-ausencia.md) | Para decidir contra o cadastro vale a consulta oficial do trilho que paga, nunca o documento; consulta sem resposta é **Extremo Perigo** com varredura que a reconsulta sozinha; e a régua distingue ausência de **cadastro** (teto de Atenção) de ausência de **identidade** (Perigo) |
| [`ADR-021`](adr/ADR-021-quatro-opcoes-de-agendamento.md) | A antecedência de 24h sai e a janela vira 9h–18h: ela é sobre a hora da SUBMISSÃO, então só bloqueia "pagar hoje" (`BLP.BIL40`); a tela oferece quatro datas prontas e o servidor as resolve |

## Como ler

Começando do zero: `01` → `02` → `07` → `03`. Para implementar uma sprint: `06` para o escopo, `02`/`07`/`11` para o modelo, `03` para as verificações, `09` para o canal de entrada, `05` para o contrato de API, `04`/`10` para a integração envolvida. `08` é a medição de campo que sustenta várias decisões — leia antes de mexer no parser ou no roteamento. `14` é a auditoria de segurança da ingestão — leia antes de mexer em parser, cota de IA, régua de risco ou dedup, para não reabrir um achado já fechado nem fechar um sem registrar. Os ADRs explicam **por que** cada decisão é assim; consulte antes de propor mudança estrutural.
