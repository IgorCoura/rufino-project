...
- **Transições de revalidação acrescentadas na 1.4**: `AwaitingApproval → Rejected` (revalidação encontrou bloqueio) e `Approved → AwaitingApproval`/`Rejected`. `BillStatus.AcceptsValidation` fecha a porta a partir de `Scheduled`, quando a verdade da execução já é da `PaymentOrder`.

- **Revalidar um boleto aprovado só derruba a aprovação quando algo mudou** (2026-09-10). Até então derrubava incondicionalmente, e isso fechava um laço sem saída: o agendamento reconfere o frescor do retrato (ADR-018), revalidar era o único jeito de renová-lo, e revalidar custava a aprovação — então agendar um boleto aprovado no dia anterior exigia aprovar de novo, todas as vezes. O que conta como "mudou" são **duas** coisas, e a segunda não é zelo:
  - o **desfecho** de qualquer das catorze verificações — comparado por `Outcome` + `Severity` + `ReasonCode`, nunca pela `Evidence`, que é texto com valores e datas dentro e mudaria em toda rodada, zerando o efeito da regra;
  - o **valor a pagar**, confrontado com `ApprovalRecord.AmountAtDecision` (coluna nova, migração `ApprovalAmountAtDecision`). As verificações podem sair idênticas enquanto o valor sobe — é o que acontece todo dia num boleto vencido, porque `AmountMatch` compara contra a política do beneficiário e não contra o número que o aprovador viu. Sem esta metade, "nada mudou" autorizaria um débito maior que o consentido. Aprovação anterior à coluna (valor nulo num boleto que tem valor) cai uma vez para a fila de decisão: sem saber contra o que a pessoa consentiu, manter a aprovação seria presumir.

  `ValidationOutcome.ApprovalPreserved` diz qual das duas coisas aconteceu — deduzir isso do status exigiria saber em que estado o boleto estava antes da chamada, que é justamente o que quem chamou já não tem.

- **Revalidar um boleto com data de pagamento escolhida é recusado** (`BLP.BIL41`, 2026-09-10). Antes a revalidação levava qualquer `Approved` a `AwaitingApproval` **sem limpar a data**, produzindo um estado impossível: aguardando aprovação com ordem em voo. Desfazer o agendamento é ato próprio, e quem o desfaz é gente.
