...
public class TaxIdTests
{
    ...
    // Parse deduz CPF pelos 11 dígitos, aceitando texto formatado.
    [Theory]
    [InlineData(VALID_CPF)]
    [InlineData(VALID_CPF_FORMATTED)]
    [InlineData(VALID_CPF_PARTIAL)]
    public void Parse_WithElevenDigits_ShouldInferCpf(string input)
    {
        var taxId = TaxId.Parse(input);

        Assert.Equal(VALID_CPF, taxId.Value);
        Assert.Same(TaxIdKind.CPF, taxId.Kind);
    }

    // Parse deduz CNPJ pelos 14 dígitos, aceitando texto formatado.
    [Theory]
    [InlineData(VALID_CNPJ)]
    [InlineData(VALID_CNPJ_FORMATTED)]
    public void Parse_WithFourteenDigits_ShouldInferCnpj(string input)
    {
        var taxId = TaxId.Parse(input);
    ...
    // Quantidade de dígitos que não é 11 nem 14 não corresponde a documento nenhum — SHK.TAX02.
    [Theory]
    [InlineData("")]
    [InlineData("   ")]
    [InlineData("123")]
    ...
    public void Parse_WithUnrecognizableLength_ShouldThrowInvalidFormat(string input)
    {
        var ex = Assert.Throws<DomainException>(() => TaxId.Parse(input));

        Assert.Equal("SHK.TAX02", ex.Id);
    }

    // Parse não relaxa o dígito verificador: comprimento certo com DV errado continua reprovado — SHK.TAX03.
    [Theory]
    [InlineData(INVALID_CHECK_CPF)]
    [InlineData(INVALID_CHECK_CNPJ)]
    public void Parse_WithWrongCheckDigits_ShouldThrow(string input)
    {
        var ex = Assert.Throws<DomainException>(() => TaxId.Parse(input));
    ...
    // TryParse aceita o que Parse aceita, devolvendo o VO já deduzido.
    [Theory]
    [InlineData(VALID_CPF)]
    [InlineData(VALID_CPF_FORMATTED)]
    [InlineData(VALID_CNPJ_FORMATTED)]
    public void TryParse_WithValidDocument_ShouldSucceed(string input)
    {
        var parsed = TaxId.TryParse(input, out var taxId);

        Assert.True(parsed);
        Assert.NotNull(taxId);
    }

    // TryParse existe para fonte que pode simplesmente não ter o documento: ausência e lixo
    // viram false, nunca exceção — é o que impede um CNPJ corrompido na resposta do provedor
    // de derrubar a consulta oficial inteira.
    [Theory]
    [InlineData(null)]
    [InlineData("")]
    [InlineData("   ")]
    [InlineData("123")]
    [InlineData(INVALID_CHECK_CPF)]
    [InlineData(REPEATED_CPF)]
    [InlineData("nao-e-documento")]
    public void TryParse_WithAbsentOrInvalidDocument_ShouldReturnFalseWithoutThrowing(string? input)
    {
        var parsed = TaxId.TryParse(input, out var taxId);
