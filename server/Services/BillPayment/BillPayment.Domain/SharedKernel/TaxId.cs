...
public sealed class TaxId : ValueObject
{
    ...
    /// <summary>
    /// Deduz CPF ou CNPJ pela quantidade de dígitos. Existe porque o documento quase nunca
    /// chega rotulado — vem de um PDF, de um campo de formulário ou da consulta oficial —
    /// e deixar a camada de cima adivinhar o tipo espalharia a regra dos 11/14 dígitos.
    /// </summary>
    public static TaxId Parse(string value)
    {
        ...
        if (digits.Length == TaxIdKind.CPF.ExpectedLength)
            return new TaxId(digits, TaxIdKind.CPF);
        if (digits.Length == TaxIdKind.CNPJ.ExpectedLength)
            return new TaxId(digits, TaxIdKind.CNPJ);

        throw TaxIdErrors.InvalidFormat(value);
    }

    /// <summary>
    /// Versão não-lançante de <see cref="Parse"/>, para quando o documento chega de uma fonte
    /// que pode simplesmente não tê-lo — a resposta de uma consulta oficial, um texto extraído
    /// de PDF. Ausência e lixo viram <c>false</c>; quem chama decide o que isso significa.
    /// </summary>
    public static bool TryParse(string? value, out TaxId? taxId)
    {
        taxId = null;
