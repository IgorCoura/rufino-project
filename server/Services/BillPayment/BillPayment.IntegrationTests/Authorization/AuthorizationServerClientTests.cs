...
public sealed class AuthorizationServerClientTests
{
    ...
    public async Task Fetch_WhenKeycloakAnswersAccessDenied_ShouldResolveToAnEmptySnapshot()
    {
        ...
        Assert.False(Grants(result, "bill#approve"));
    }
    ...
    public async Task Snapshot_WhenAllRequestedScopesArePresent_ShouldGrant()
    {
        ...
        Assert.True(Grants(result, "capture-source#manage,sync"));
    }
    ...
    public async Task Snapshot_WhenARequestedScopeIsMissing_ShouldDeny()
    {
        ...
        Assert.False(Grants(result, "capture-source#manage,sync"));
    }
    ...
    {
        var client = CreateClient(new StubHandler(_ => Response(HttpStatusCode.OK,
            "[{\"rsid\":\"1\",\"rsname\":\"bill\",\"scopes\":[]}]")));

        var result = await client.FetchAllPermissionsAsync();

        Assert.True(Grants(result, "bill#"));
        Assert.False(Grants(result, "bill#approve"));
    }
    ...
    {
        var client = CreateClient(new StubHandler(_ => Response(HttpStatusCode.OK,
            "[{\"rsid\":\"1\",\"rsname\":\"bill\",\"scopes\":[\"view\"]},"
            + "{\"rsid\":\"1\",\"rsname\":\"bill\",\"scopes\":[\"approve\"]}]")));

        var result = await client.FetchAllPermissionsAsync();

        Assert.True(Grants(result, "bill#view,approve"));
    }
    ...
    {
        var client = CreateClient(new StubHandler(_ => Response(HttpStatusCode.OK,
            "[{\"rsid\":\"1\",\"rsname\":\"bill\",\"scopes\":[\"approve\",\"approve-attention\"]}]")));

        var result = await client.FetchAllPermissionsAsync();

        var granted = result.Snapshot.GrantedScopes("bill", ["approve-extreme", "approve-danger", "approve-attention"]);
