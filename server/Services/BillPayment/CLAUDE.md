...
2. `IBillRepository.ExistsActiveByDigitableLineAsync(line)` — a unicidade da linha digitável é **global**, com índice único **sem `TenantId` na chave** (comente isso no mapping EF, senão alguém "conserta" depois).
3. `IRoutingRuleRepository.ExistsForPairInAnyTenantAsync(payeeTaxId, accountRef)`.
...
**Sprint 2.3 — 🚧 Em andamento.** As portas e a regra de triagem existem; **os adapters ainda não**. O que aterrissou:
...
**Falta**: a purga por desfecho dos itens que já existem (o handler já não guarda o que não é boleto), o job que dispara o processamento, e a 2.4.
...
- **`CaptureSource.FolderPath` é opcional e NÃO é mecanismo de triagem.** O padrão é a caixa de entrada inteira, e assim deve continuar: **a triagem é trabalho do software**, não do usuário. Exigir que alguém mova boleto para uma pasta à mão devolveria ao usuário exatamente a tarefa que este BC existe para eliminar — decisão do usuário em 2026-08-11, corrigindo enquadramento anterior. A pasta serve a quem *já* tem caixa dedicada, ou quer restringir o escopo por conta própria; nunca é pré-requisito de sprint.
...
- **Mensagem sem anexo utilizável não vira `CaptureItem`** — inclusive a que traz o boleto no corpo ou por link, que é caso da sprint 2.5. **Nada se perde em definitivo**: `ResetCursor` faz o Graph devolver tudo que ainda está na caixa, então a 2.5 recupera o histórico ao entrar.
...
- **`CursorCodec` (`Application/Queries/`) é a única implementação do cursor de keyset.** Os mesmos ~28 linhas viviam copiados byte a byte dentro de `BillQueries`, `PayeeQueries` e `TrustedOriginQueries`. Trocar a codificação com N cópias exigiria achar todas, e a que ficasse para trás devolveria página errada **em silêncio** — o pior modo de falhar numa paginação. Cursor inválido devolve `false` e a lista começa do início, em vez de derrubar a tela com 500.
...
│   ├── BackgroundServices/           #   CaptureSyncBackgroundService + CaptureSyncOptions (Capture:Enabled, off por padrão)
...
│   ├── CaptureSources/Commands/      #   Connect (prova de acesso + cofre + aviso ADR-008), Rename, AlterActivation, ReplaceCredential, Disconnect
...
│   ├── Services/                     #   BillValidationService, PayeeResolutionService, BillValidationContext, CaptureTriageService, PasswordDerivationService (Domain Services estáticos)
...
│   ├── Extraction/                   #   ExtractionResult, PasswordCandidate, ExtractionErrors
│   ├── Ports/                        #   IBankDirectory, IBillLookupService, IPixLookupService, ISecretVault, IMailboxReader — contratos de mundo externo (não SeedWork)
...
│   ├── Extraction/                   #   CandidateScanner, QrCodeScanner (ZXing), PdfBoletoDocumentParser (PdfPig), ExtractionOptions
...
├── BillPayment.UnitTests/            # xUnit — 735 testes; pasta por Aggregate, cada uma com Mothers/
├── BillPayment.IntegrationTests/     # xUnit + Testcontainers + Respawn — 237 testes
...
└── BillPayment.Architecture/         # design rationale do BC — fonte de verdade do modelo
    ...
    ├── tools/                        #   analyze-boleto-corpus.js, probe-asaas-simulate.js, smoke-probe-{production,pix-decode,mailbox}.js, fetch-bacen-participants.js
    └── adr/                          #   ADR-001..014
```
...
- [ ] **`Storage:ServiceUrl`, `AccessKey` e `SecretKey`** — balde compatível com S3 (Garage) para os artefatos capturados. **Sem isso o processamento de anexo falha alto**, de propósito: é preferível a guardar em lugar nenhum e descobrir na auditoria. O segredo vai por variável de ambiente, nunca no `appsettings.json`.
- [ ] **Master key do cofre (`Secrets__MasterKey`)** — 32 bytes em base64, gerada por `SecretsOptions.GenerateMasterKey()`. Sem ela o BC sobe com um cofre que falha em toda operação; a Fase 1 tolera isso porque não guarda credencial de tenant, mas **a partir da fase 2 é pré-requisito de deploy**. Guarde uma **cópia cifrada fora do host** (`age`/`gpg`) — perdê-la é reconectar todas as caixas e reemitir todas as chaves de subconta (ADR-009).
...
- [ ] **Assets nativos do SkiaSharp no contêiner** — o leitor de QR decodifica imagem via SkiaSharp, que precisa de bibliotecas nativas no Linux. A imagem base do Dockerfile pode não trazê-las, e a falha apareceria só em runtime, como QR silenciosamente não lido. **Validar rodando a cascata dentro do contêiner** antes do deploy; se faltar, acrescentar `SkiaSharp.NativeAssets.Linux` e as dependências de sistema.
