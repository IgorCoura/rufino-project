...
1. **A senha derivada funcionou com volume: 11 PDFs cifrados abriram.** E **7 dos 11 abriram por documento _adicional_**, 5 deles pelo **CPF** — `cnpj_first_5_primary` (4), `cpf_first_3_additional_1` (3), `cpf_first_5_additional_1` (2), `cnpj_first_5_additional_0` (2). Derivar só do `PrimaryTaxId` teria perdido 7 boletos; os prefixos curtos dominam, como o doc 09 previa. Confirma com volume o que o corpus sugerira com um caso só. **Os prefixos 3 e 4 entraram depois, em 2026-09-10** (achado do usuário): um boleto do acervo abre com os QUATRO primeiros dígitos do CNPJ, e como o prefixo não estava na lista a senha certa nunca era oferecida — o documento virava `pdf_locked`, que se lê como "a senha não bate" quando a verdade era "ninguém tentou a senha certa". Esta medição só tinha visto 5 e 3, e a ausência do 4 passou por regra em vez de por amostra pequena. **O teto de candidatas subiu de 40 para 60 no mesmo commit**: dois prefixos a mais por documento truncariam o fim da fila, que é onde vivem os documentos adicionais que abriram 7 dos 11. E **prefixo curto deixou de valer como prova forte no degrau 0** — abaixo de `PasswordCandidate.STRONG_PREFIX_MIN_LENGTH` (5) a promoção é `Weak` (`password_derived_short_prefix`), a menos que o documento do tenant também esteja impresso na página: três dígitos são mil valores, e acerto pode ser coincidência.
...
4. **A visão aceita imagem, o parser determinístico não.** `image/png`, `image/jpeg`, `image/webp` e PDF. É a correção do buraco medido: **12 anexos recusados com `not_a_pdf`**, baixados e nunca lidos. Quem decide qual dos quatro é **`DocumentMagic`, pelos bytes** — `application/octet-stream` só cai em PDF por convenção quando nenhuma assinatura é reconhecida.
...
⚠️ **O rótulo de mídia é declaração de terceiro — quem decide são os bytes (`DocumentMagic`).** Bug de 2026-09-10: o boleto da **Notredame Intermédica** chega com `Content-Type: pdf`, que não é tipo de mídia válido (falta o `application/`). O Exchange normaliza o que não entende para `application/octet-stream`, e esse rótulo atravessava ingestão → balde → API até a tela, onde o app — que só abre `application/pdf` e `image/*` — mostrava **"Este documento chegou em um formato que o app não exibe"** sobre um PDF que abre em qualquer leitor. É a terceira encarnação do erro de 2026-08-11: guardar o que o provedor declarou **não torna a declaração verdadeira**. Três lugares passaram a medir: `DocumentPayload.From` (os bytes ganham até de um rótulo declarado e suportado — um PNG anunciado como PDF ia rotulado errado para a visão), `ProcessCaptureItemCommand.ApplyAsync` (o balde recebe o tipo medido, não o declarado) e **`ArtifactDownload.OpenAsync`**, que é o que conserta **o que já está guardado sem migração de balde**. Como o fluxo do S3 não volta atrás, só `DocumentMagic.PREFIX_LENGTH` (12) bytes são espiados e devolvidos à frente do fluxo pelo `PrefixedStream` — o download continua em streaming. Efeito colateral que também era bug: `UnlockedArtifactReader` só destrava o que ele acha que é PDF, então boleto cifrado rotulado `octet-stream` chegava **trancado** ao aprovador. Regressões em `DocumentMagicTests` e `ArtifactDownloadTests`.
...
- **`Domain/Extraction/`**: `DocumentLink` (link já desembrulhado, com host **e porta**), `ResolvedDocument` (bytes + mídia + URL de origem) e **`DocumentMagic`** (o tipo de mídia lido nos bytes — PDF, PNG, JPEG, WEBP —, fonte única para a escada de link, o `DocumentPayload` e o download). `ExtractionErrors` foi para `BLP.EXT01–07`.
...
- **API**: `PaymentsController` (list/detail/by-bill + `cancel` + `confirm-immediate`) **reusando escopos `bill`** de propósito (`view`/`cancel`/`approve` — mesma doutrina do `capture-item:claim` na reprovação; nenhum partial import novo no realm); `PaymentSubmissionBackgroundService` (**serial**, ligado por padrão `PaymentSubmission:Enabled`, com a janela 9h–18h valendo só para o pagamento que executa hoje — 17h até o ADR-021, e fila inteira até 2026-09-10; a reconferência das ordens `AwaitingAccount` roda sempre) + `PaymentSubmissionOptions`. Seções novas no `appsettings`: `Payments` (política ADR-017) e `PaymentSubmission` (worker). A suíte desliga `PaymentSubmission:Enabled` na fábrica.
...
- **Comprovante**: `CaptureReceiptOnPaymentPaidHandler` (segundo handler do `PaymentOrderPaidDomainEvent` — **registrar handler novo em `ApplicationDependencies` é obrigatório**, o dispatcher só resolve o que o DI conhece; esquecê-lo foi exatamente o defeito pego pelo teste) → `CapturePaymentReceiptCommand`: `GET` fresco no provedor (a URL é credencial ao portador e **nunca é persistida nem logada** — só o host), download via porta **`IPaymentReceiptFetcher`** (adapter `HttpPaymentReceiptFetcher`, cliente `asaas-receipt` sem retry). **A URL do provedor serve a PÁGINA do comprovante, não o arquivo (medido em 2026-09-09)**: o adapter atravessa a âncora `/transactionReceipt/pdf/{id}` que a página oferece — **um salto, mesmo host e mesma porta**, e o tipo é decidido pelo `%PDF-` do corpo, nunca pelo cabeçalho. Sem link, a página continua sendo guardada (sem regressão). Enquanto isso não existia, o balde recebia `comprovante-{id}.html` e o app mostrava "formato que o app não exibe" — defeito sem sintoma nenhum no servidor, detalhado no `gotchas.md`. **A falha do comprovante NÃO derruba mais o espelho do boleto (2026-09-09)**: o `OutboxProcessor` despacha todos os handlers de um evento na MESMA transação, e este roda depois do espelho — o `BLP.PMO21` revertia a transação inteira, levando junto o boleto que já tinha virado `Paid`, e cinco tentativas depois ia para dead-letter (ordem `Paid`, boleto `Scheduled`, em silêncio). O handler de evento agora ENGOLE e registra; quem persegue o arquivo é a varredura `ClaimPaidMissingReceiptAsync`, com backoff INFINITO contra o finito do outbox. **Estado de boleto não depende de PDF** — o comando continua lançando, o que muda é quem o deixa passar, gravação no balde e `AttachReceipt`. Servido por `GET /payments/{id}/receipt` (`bill:view`, 404 colapsado).
...
**Auditoria 2026-09-03 — onda 3 (robustez e convenção).** As regras que se somaram às da onda 1:

- **Tradutor de status ÚNICO**: o mapa wire→status vive em **`ProviderStatusCatalog`** (Domain, função pura) — o `AsaasPaymentStatusMap` da Infra e o webhook DELEGAM para ele. Não reintroduza um switch local: dois mapas divergem em silêncio.
- **Consulta de idempotência nunca produz NotFound por falha**: nos gateways, `FindByExternalReference` só devolve `NotFound` com lista vazia DEFINITIVA (200) — qualquer falha, retentável ou não, degrada para `Unavailable` (NotFound indevido autoriza reenvio = pagamento em dobro). No `GetAsync`, só o **404** afirma ausência (`AsaasFailure.IsNotFound`).
- **O ternário de trilho mora em `PaymentGatewayRouting`** (Application) — cancel/fetch/get roteiam por lá nos seis sites; a adoção por referência preenche `Amount` do retrato do provedor (`ProviderPaymentSnapshot.Amount`).
- **Rede de segurança do comprovante**: a conciliação também varre `Paid`/`Refunded` sem `ReceiptStorageKey` — **ou com chave terminada em `.html`, que é a página gravada no lugar do arquivo e conta como ausente** (`LandingPageReceipt`, na Application, é o único dono dessa regra: o handler a lê em C# e a varredura a recebe como parâmetro do `LIKE`, para ela não existir em dois lugares). É o que repesca o acervo capturado antes de 2026-09-09; a página antiga sai do balde **depois** do save, e repescagem que devolve página de novo **não regrava** — via **`ClaimPaidMissingReceiptAsync`** e redispara `CapturePaymentReceiptCommand` com **`Definitive: true`** — sem URL no provedor nessa segunda olhada, `MarkReceiptMissing` grava **`ReceiptUnavailable`** e tira a ordem da varredura (o caminho do outbox NÃO grava a marca: o arquivo pode só não existir ainda). `AttachReceipt` limpa a marca. Migração `PaymentOrderReceiptSweep` (`receipt_unavailable`, `sweep_attempted_at`).
- **Claims de varredura carimbam `sweep_attempted_at` na SAÍDA** (ADO, como a contagem de tentativa da fila) e ordenam nunca-tentadas primeiro — é o anti-inanição: ordem que falha conciliação repetidamente roda para o fim do lote em vez de monopolizar as 50 vagas. Os dois claims dividem a MESMA coluna sem interferência (status disjuntos).
...
⚠️ **Ressalva desta entrega, corrigida em 2026-09-09 — leia junto com a seção daquele dia.** O
texto original dizia que a suíte de integração ficara com 21 falhas **pré-existentes** do trabalho
não commitado do ADR-018/ADR-019 e que *"HEAD roda 739/739 verde"*. **As duas metades eram
verdadeiras no dia e enganosas juntas**: a ressalva descrevia algo transitório que virou
permanente, e o número medido num commit anterior continuou parecendo atual — quatro commits
depois ninguém sabia que havia dívida. As falhas sobreviveram até 2026-09-09, quando as 11 que
restavam foram diagnosticadas e consertadas (`gotchas.md` → "suíte vermelha herdada vira suíte
ignorada"). Duas lacunas daquele trabalho já haviam sido corrigidas aqui porque bloqueavam a
...
## 2026-09-15 (2) — O boleto importado à mão nunca sabia quem era o pagador

Relatado sobre o `RUF101 - JUNDDIAMONDS.pdf`: a verificação 8 dizia "não foi possível determinar de
quem é este documento" sobre um boleto do Itaú que imprime, embaixo, `Pagador: RUFINO
EMP.ELET.HIDR.LTDA  CNPJ/CPF: 02.624.917/0001-92`.

**A leitura não tinha nada de errado** — rodando o parser real sobre o arquivo, o `TaxIdScanner`
devolve `02624917000192` com `UnderPayerLabel = true` e o beneficiário sem rótulo, exatamente como
deveria. O dado era **descartado no handler**: `ImportBillCommandHandler` já roda a cascata (e até
passa os documentos do cadastro), usava só `extraction.Instruments` e chamava `Bill.Capture` sem
`extractedPayer`. Todo boleto importado pela tela nascia com `ExtractedPayer` nulo, e o check 8 caía
no ramo `payer_not_extractable` para sempre — a leitura por IA não salva, porque ela preenche
`Reading.PayerTaxId`, que o check 8 não lê.

- **`BillRoutingService.IdentifyPayer(extraction, profile)`** — o mesmo critério do `Route` exposto
  para quem **já sabe de quem o boleto é**: documento do tenant impresso; na falta dele, o que veio
  sob rótulo de pagador. Fica no Domain Service de propósito — é decisão sobre dois agregados, e
  copiá-la para o handler seria a régua escrita em dois lugares. `Route` passou a usar o mesmo
  `OwnParty` interno, então as duas não podem divergir.
- ⚠️ **Candidato sem rótulo e fora do cadastro continua não virando pagador.** Devolver "o primeiro
  documento fiscal do papel" faria o CNPJ da concessionária contradizer o cadastro e **bloquear** um
  boleto legítimo (ADR-004) — é o inverso do defeito, e pior.
- **Devolver o pagador de outra pessoa é desfecho desejado**: o documento que nomeia outro pagador
  sob rótulo passa a bloquear no check 8 (`payer_mismatch`) em vez de silenciar. Quem importou vê o
  motivo; recusar a importação continua não sendo decisão desta leitura.
- **A reivindicação continua passando `extractedPayer: null`** — lá é coerente por construção (item
  com pagador identificado não chega a ser reivindicável, `BLP.CPI04`). Fica a lacuna conhecida do
  item que foi para a fila **antes** de o cadastro fiscal existir: relendo hoje, o pagador seria
  achado e continua sendo jogado fora.
- ⚠️ **Boleto importado antes do deploy precisa de "Revalidar"** — e o pagador só passa a existir
  se ele for reimportado; o campo é gravado na captura, e revalidar não relê o arquivo.

Testes: 5 em `Services/BillRoutingServiceTests` (o **teste-âncora** do documento do tenant impresso,
o cadastro vencendo o rótulo quando a varredura rotulou o outro, o rotulado de terceiro que precisa
chegar ao check, a **contraprova** do candidato sem rótulo e fora do cadastro, e o tenant sem
cadastro) e 3 em `Bills/ImportBillManualTests` (o **teste-âncora** do pagador impresso chegando ao
boleto, a **contraprova** do CNPJ do beneficiário sozinho não virando pagador, e a importação sem
perfil fiscal ainda lendo o rotulado).

## 2026-09-15 (3) — O documento é RELIDO a cada validação, e a verificação 8 deixa de depender da captura

Pergunta do usuário depois do conserto do import: *"as verificações deveriam ser totalmente
independentes, obtendo suas próprias informações, sem receber informações de processos antigos que
não podem mais ser rodados — se não, a revalidação não funciona por completo, e plugar outras fontes
de boleto também não vai funcionar."* Verificado: das catorze, **onze já eram independentes** — o
`ValidateBillCommandHandler` remonta o contexto inteiro a cada rodada (consulta oficial dos dois
trilhos, beneficiário, origem, perfil fiscal, expectativas, duplicata, boletos substituídos, a data).
As três que não eram: **8** (`ExtractedPayer`), **13** (`Reading`) e **11** (`Routing`).

A 8 foi separada; **as outras duas ficam como estão**, por decisão do usuário — a 13 porque a leitura
por IA custa dinheiro e tem teto por tenant, e a 11 porque "por qual degrau este boleto foi
atribuído" é fato histórico, como o remetente.

- **`IBillDocumentSource`** (`Application/Bills/`) — o irmão **determinístico** do
  `IBillReadingSource`: abre o documento guardado, destrava com senha derivada e roda a cascata
  barata (texto + QR + varredura de documentos fiscais). Sem IA, sem custo de provedor. **Medido em
  33 ms** (mediana, boleto de uma página, 18 KB) — por isso pode rodar a cada validação, como a
  consulta oficial roda.
- **`BillValidationContext.DocumentPayer` + `DocumentReread`**, preenchidos pelo handler — o mesmo
  lugar por onde as consultas oficiais entram. O Domain Service continua **puro e síncrono**.
- **A verificação 8 lê a releitura quando ela aconteceu**, e o que o agregado guardou quando não
  aconteceu. As duas metades importam: releitura que devolve "nenhum pagador" é **afirmação** e
  sobrepõe o retrato; releitura que não rodou é **ausência de resposta** e não apaga nada.
- **`Bill.RefreshExtractedPayer`** — o campo deixou de ser retrato da captura e virou **cache da
  última leitura**, reescrito a cada validação. Continua existindo para a lista e o detalhe
  mostrarem algo antes da primeira validação, e para o boleto sem documento guardado.
- ⚠️ **Falha fechada, nunca falha aberta**: balde fora do ar, chave órfã, PDF que nenhuma senha abre
  e tipo de mídia que a cascata não lê viram "não foi relido" — a apuração das catorze **nunca** cai
  por causa do documento. `BillDocumentSource.ReadAsync` captura tudo que não é cancelamento e
  registra; o que **não** entra no log é o conteúdo lido.
- **O cadastro fiscal entra como está HOJE**: o documento que o tenant cadastrou depois da captura
  passa a ser reconhecido sem reimportar o boleto.

**O ganho que responde à segunda metade da pergunta:** fonte nova de boleto precisa apenas **guardar
os bytes e a origem** — as verificações se viram sozinhas. Era exatamente o esquecimento do import
que produziu o defeito do RUF101, e ele deixa de ser possível por construção.

⚠️ **Boleto importado antes disto se conserta com "Revalidar"** — e não mais só reimportando, que era
a ressalva da entrega anterior.

Testes: 6 unitários (4 em `Services/BillValidationServiceTests` — o **teste-âncora** da releitura que
identifica o pagador com o agregado vazio, a releitura que desmente o retrato guardado, a que afirma
"ninguém" e a **contraprova** de que sem releitura vale o que o agregado tem; 2 em
`Bills/BillValidationTests` para o método rico) e `Bills/BillDocumentRereadTests` (4, pela borda — o
boleto gravado sem pagador curado pela revalidação, o cadastro fiscal chegando depois, a contraprova
do boleto sem documento guardado, e o documento que não nomeia ninguém).
...
inteiro ou só a primeira página**, **anexar ou não os comprovantes** e **um PDF único ou um PDF por
boleto**. Um endpoint novo, leitura pura, e zero migração.

**`POST /api/v1/{tenantId}/bills/documents/export`** (`bill:view`, policy `expensive`, sem
`x-requestid` — é `POST` só porque a lista de ids estouraria a URL). Corpo:
`{ billIds, pages: All|FirstPage, includeReceipts, packaging: SinglePdf|PdfPerBill }`. Responde o
arquivo: `application/pdf`, ou `application/zip` com um PDF por boleto. Cada boleto incluído deixa
uma linha de `ArtifactAccessLog` (`bill-export`, ou `bill-export-payment-code` quando o código foi
escrito no aviso).

Decisões do usuário que o código fixa, cada uma com teste em `Bills/BillDocumentExportTests`:

1. **"Somente a primeira página" corta só o documento do boleto** — o comprovante sai inteiro.
2. **Um PDF por boleto sai num `.zip`**; com um boleto só, o PDF vem direto.
3. **Boleto sem documento vira página de aviso** com a procedência do código (e-mail com remetente,
   assunto e data; inserido manualmente; portal) e **o próprio código de pagamento** — a linha
   digitável formatada como é impressa e o Pix copia e cola. Documento guardado que não abre (senha
   que o sistema não conhece, arquivo corrompido) recebe o mesmo aviso. Boleto pago sem comprovante
   guardado (ou com a página HTML antiga do provedor) recebe o aviso "Comprovante indisponível";
   boleto nunca pago não recebe nada.
4. **A ordem do arquivo é a ordem em que os boletos foram marcados** — o cliente manda os ids nessa
   ordem e o servidor não reordena.

⚠️ **A página de aviso escreve a linha digitável e o Pix, e isso é EXCEÇÃO DELIBERADA à regra de que
os dígitos não saem pela API** (decisão do usuário, 2026-09-14). O portão é o mesmo do documento
original (`bill:view`), e o documento original já traz o código impresso; sem o aviso, o boleto
importado à mão seria o único que a exportação não conseguiria entregar. **Não estenda a exceção a
nenhum outro endpoint** — `GET /bills`, `GET /bills/{id}/detail` e o resto continuam sem os dígitos.
...
- **`Application/Queries/Bills/BillDocumentExportQueries`** + `BillDocumentExport.cs` (Smart Enums
  ...
  **`UnlockedArtifactReader`** — PDF cifrado entra destravado, como no visualizador — e o comprovante
  pelo balde. `BillQueries.BeneficiaryOf` virou `internal` para o nome do arquivo e o aviso usarem
  a mesma guarda contra o pagador lido como beneficiário.
- **Erros novos**: `BLP.BIL42` (seleção vazia), `BLP.BIL43` (mais de 50 boletos), `BLP.BIL44`
...
Relatado sobre um e-mail com duas **GFD — Guia do FGTS Digital** (o modelo que substituiu a GRF)
que foram parar na fila de reivindicação em vez de virar boleto. Rodando o PDF real pelo caminho do
código (PdfPig 0.1.16 + ZXing/SkiaSharp, as versões da Infra), a cadeia inteira ficou visível — e
**três camadas empilhadas**, das quais só a do meio é nossa.

### O que a GFD entrega, e o que ela não entrega

- **Instrumento**: só **QR Pix dinâmico** (`cobv` da Caixa). Não há código de barras nem linha
  digitável — o modelo novo é Pix-only. O QR é lido normalmente (`decoded=1`, CRC confere), então a
  cascata **resolve** e a triagem devolve `Parse`.
- **Pagador**: a guia imprime `CPF/CNPJ do Empregador: 02.624.917` — **só a raiz do CNPJ**, 8
  dígitos. Os 14 não aparecem em lugar nenhum do documento; o campo `Tag` repete a mesma raiz.
- **Texto**: as três fontes embutidas são `Type0`/`CIDFontType2` com `/Encoding /Identity-H`,
  **zero `/ToUnicode`**, e os arquivos TrueType embutidos trazem só
  `['cvt ','fpgm','glyf','head','hhea','hmtx','loca','maxp','prep']` — **sem `cmap`, sem `post`**.
  Não existe mapa glifo→caractere no arquivo. `page.Text` volta como mojibake com **zero dígitos**
  (`pdftotext` sai embaralhado do mesmo jeito). É defeito do gerador da Caixa/FGTS, não nosso, e
  **nenhum extrator de texto resolve** — só visão/OCR.

Consequência: `CandidateScanner` e `TaxIdScanner` voltam vazios, `ExtractionResult.Parties` fica
vazio, e `BillRoutingService` cai no degrau 4 — `Unrouted` / `payer_not_identified`.

### 🐛 O defeito nosso: o pagador lido pela visão era descartado quando o determinístico resolvia

`ProcessCaptureItemCommand.ExtractWithVisionAsync` tinha um `return` antecipado:
...
A IA **rodava** (desde 2026-08-27 ela roda para todo candidato a boleto) e lia o pagador — mas só o
retrato de enriquecimento sobrevivia; o `extracted.PayerTaxId` era jogado fora, e os `parties` da
visão só entravam na escada no ramo de baixo, o de quem **não** resolveu. Ou seja: **documento cujo
instrumento vem do QR e cujo pagador só existe na visão nunca subia ao degrau 1**.

Não é exótico — é exatamente a forma da GFD, e de qualquer documento com QR legível e camada de
texto ilegível. `ExtractionResult.WithParties` funde os candidatos preservando os que a varredura
determinística já lera (a ordem importa: é a ocorrência **rotulada** que sustenta o degrau 1
negativo), deduplicando por `(documento, rótulo)` como o `TaxIdScanner`, e mantendo
`Method`/`UnlockedBy`/`ReasonCode` — fundir documento fiscal não pode reescrever de onde veio o
instrumento, que é métrica da cascata.

🔑 **A fusão é estritamente aditiva, e a assimetria do ADR-011 continua de pé**: a visão entra com
`underPayerLabel: false`, então ela só **promove** (casando com o cadastro do tenant) e nunca
declara "é de outra pessoa". Não há como esta mudança rotear boleto alheio para cá — tem
contraprova nos dois níveis.

### ⚠️ O que isto NÃO resolve, e por quê

**A raiz do CNPJ continua inalcançável pelo caminho determinístico.** `TaxIdScanner` só emite
sequências de exatamente 11 ou 14 dígitos (ou o documento cadastrado encontrado dentro de uma
sequência maior), `PartyCandidate.TryCreate` exige DV válido, e `PayerProfile.OwnsByCnpjRoot` recebe
um `TaxId` completo — não há representação para uma raiz solta. Na GFD isso é irrelevante, porque a
camada de texto não entrega dígito nenhum; passa a importar no dia em que a visão estiver desligada
ou a cota esgotada. **Quem fecha essa lacuna hoje são os `ExtractionHints`**, que levam ao modelo os
documentos do próprio tenant — é o caso de "campo cortado" para o qual eles existem.

**O Pix dinâmico não carrega valor nem vencimento** (não há tag 54; o valor vive na URL da `cobv`).
Promovida a `Bill`, o valor e a data vêm do retrato da IA, não do instrumento, e o check de valor
sai `Skipped`. É o comportamento correto do trilho, não regressão — mas é o que vigiar na GFD.

**Testes:** 1.402 unitários (+9) e 862 de integração (+2) verdes; `dotnet build BillPayment.sln
-p:TreatWarningsAsErrors=true` limpo. O teste-âncora é
`VisionExtractionTests.Process_WhenTheCascadeResolvedAndOnlyVisionReadsThePayer_ShouldPromoteAsStrong`
— **provado pelo avesso**: restaurado o `return` antecipado, ele fica vermelho. Vêm junto a
contraprova de que o terceiro lido pela visão continua indo para a reivindicação,
`Extraction/ExtractionResultTests` (7, novos — a fusão, a preservação do que já havia, a
deduplicação por documento E rótulo, e o desfecho não resolvido que preserva o motivo) e dois em
`BillRoutingServiceTests` no nível da escada.

## 2026-09-10 (6) — A fila dormia com o pagamento de amanhã na mão

Relatado como dois boletos agendados para o dia seguinte parados havia mais de uma hora em "Na
fila de envio". Não era defeito: `RunCycleAsync` conferia a janela de submissão (9h–18h,
`America/Sao_Paulo`) e **abortava o ciclo inteiro** fora dela, então uma ordem criada às 21h
esperava até as 9h da manhã seguinte.

**A decisão do usuário derruba a premissa, não a janela**: enquanto a ordem espera aqui, não há
agendamento em lugar nenhum — uma queda nossa a perde. Submetida, ela vira agendamento **no
provedor**, que sobrevive a essa queda. Segurar era o risco.

A janela passou a valer para o pagamento que **executa hoje**, e o recorte mora na
**reivindicação** da fila (`ClaimPendingSubmissionsAsync`, que ganhou `today` e
`submissionWindowOpen`), nunca no `SubmitPaymentOrderCommand` — a tentativa é contada na saída da
fila, então reivindicar o que não seria submetido gastaria tentativa e levaria a ordem à
desistência sem ela nunca ter falado com o provedor.

🔑 **Boleto VENCIDO continua esperando a abertura, mesmo pedido para outro dia.** O provedor
processa conta vencida **na hora, ignorando a data** (premissa do ADR-017, regra 3): às 23h isso
não é agendamento, é dinheiro saindo sem ninguém acordado para reagir ao alerta. É por isso que
a consulta do claim lê `bills.due_date` — é o único ponto em que o vencimento pode ser
consultado a tempo de **não** reivindicar a ordem. O `FOR UPDATE` continua travando só
`payment_orders`; o boleto é leitura de uma coluna. Boleto ausente (ordem órfã) não impede: quem
recusa é a submissão, com falha visível.

**O que NÃO mudou**: a janela (9h–18h, `Payments:SubmissionWindowStart/End`, fim exclusivo), o
bloqueio de "pagar hoje" fora dela (`BLP.BIL40`), a ordem das perguntas do veredito e as quatro
sugestões da tela. Quem agenda continua sem poder escolher hoje fora do horário; o que sumiu foi
a espera de quem escolheu outro dia. Nenhuma migração, nenhuma mudança de contrato HTTP.

De passagem, o XML doc do worker ainda dizia "9h–17h", número que o ADR-021 aposentou em
2026-09-08 — era a linha que alguém leria para responder justamente esta pergunta.

### A outra metade: hoje continua bloqueado, e agora a tela sabe disso antes

Perguntado no mesmo dia: escolher **hoje pelo seletor livre**, fora do horário, tem de ser
recusado igual — o bloqueio é da DATA, não do controle da tela que a escolheu.

**No servidor já era.** A guarda vive em `Bill.Schedule` (`BLP.BIL40`) e os DOIS caminhos de
escrita — `POST /schedule` e `POST /approve` com data — resolvem o mesmo veredito antes de
chamá-la. O que faltava era **prova pela borda**: a recusa só era exercitada no agregado, e um
handler que esquecesse de resolver o veredito passaria pela suíte inteira.
`Bills/SameDaySchedulingWindowTests` fecha isso, com a janela entrando **por configuração** em
posição conhecida em relação ao relógio (com a janela real, o desfecho dependeria da hora em que
a suíte roda).

🐛 **A prévia do seletor livre, essa sim, mentia.** `SchedulePreviewDto.For` chamava só
`PaymentSchedulingService.Resolve`, que **não conhece a hora** — então para hoje fora da janela
ela respondia "pagamento será executado em <hoje>", verde, e o 409 só aparecia no submit. As
quatro sugestões consultavam `CanScheduleForToday` desde o ADR-021; a data digitada à mão, não.
Era a assimetria que o próprio ADR-021 proíbe: *"sugestão que a tela oferece e o servidor recusa
é a tela levando alguém a um erro já conhecido"*. A prévia ganhou `Available` +
`UnavailableReason` — **mesmo vocabulário das sugestões, porque é o mesmo veredito** — e a
pergunta só é feita quando a data pedida É hoje (data futura não depende da hora).

No cliente, `SchedulePreview.available` **trava o Autorizar**, e a distinção que sustenta as duas
metades é: **prévia ausente não bloqueia** (falhar ao buscá-la é rede, não recusa), **prévia que
diz "não dá" bloqueia**. Campo ausente lê como `true`: servidor mais antigo mantém o
comportamento de antes, com a recusa acontecendo na escrita.

**Testes:** 1.393 unitários, 860 de integração (+12) e 411 de cliente (+2) verdes, build com
`TreatWarningsAsErrors` limpo. Em `Payments/PaymentOrderWorkQueriesTests` (+5) o **teste-âncora**
é o do boleto vencido pedido para outro dia — provado pelo avesso: removida a leitura do
vencimento, ele fica vermelho; junto vêm as duas metades do recorte (data futura submetida com a
janela fechada; pagamento de hoje esperando) e a **contraprova** do boleto no prazo, sem a qual
uma consulta que excluísse todo boleto passaria no âncora sem fazer nada de útil. Em
`Bills/SameDaySchedulingWindowTests` (+7): hoje recusado nos dois caminhos de escrita (com a
aprovação caindo junto, que é a atomicidade do "aprovar e agendar"), as duas contraprovas
(dentro do horário, e data futura fora dele) e as três da prévia. No cliente, o par que fixa a
regra do bloqueio e o da prévia ausente que **não** bloqueia.
...
1. **Só `AcceptsSilentRevalidation`** (`Captured`/`AwaitingApproval`). Revalidar um `Approved`
   deixou de derrubar a aprovação incondicionalmente (2026-09-10), mas **este** caso é
   justamente o que a derruba: a fila existe para boleto cujo `LookupAvailability` reprovou, e
   quando a consulta volta a responder o desfecho DAQUELE check muda — então a aprovação
   cairia, em silêncio, por obra de um worker. A regra não mudou; o motivo ficou mais preciso.
...
- **Novo (C1)** — a imagem era aceita pelo `Content-Type` que o servidor **remoto** declarava,
  enquanto o PDF sempre foi conferido pelo `%PDF-`. `DocumentMagic` fecha a assimetria: PDF, PNG, JPEG
  e WEBP pelos bytes, num lugar só. SVG fora de propósito — é XML executável, não imagem rasterizada.
...
O portão de pré-push encontrou **11 falhas** na suíte de integração. Um worktree em `4e51af43`
devolveu as mesmas 11, com os mesmos nomes: **pré-existentes**, e não regressão desta entrega
(totais 787→796, 776→785 — as 9 novas passavam todas). Nenhuma era defeito de código *aparente*;
duas eram, e uma delas movia dinheiro.

### 🐛 O dublê de autorização não fechava porta nenhuma (2 falhas)

`MockProtectedResourceHandler` fazia `context.Succeed(requirement)` **incondicionalmente**, então
`[ProtectedResource]` era decorativo na suíte inteira: nenhum teste jamais provou uma NEGATIVA de
escopo, e `EndpointProtectionTests` provava só que o atributo **está escrito**. Agora o dublê
resolve o retrato do header `bp_scopes` pela mesma `RptSnapshot.Grants(permission,
options.ScopesValidationMode)` da produção, e concede tudo apenas na ausência do header.
`FakeAuthorizationServerClient.ModelledResource`/`ScopesFrom`/`SnapshotFrom` são a regra num lugar
só. **Provado pelo avesso**: trocar `bill:schedule` por `bill:view` no endpoint deixa o teste
vermelho.

### 🐛 O evento de cancelamento carregava Smart Enum, e o outbox não sabia lê-lo (3 falhas)

**Este é o defeito de produção da lista.** `PaymentOrderCancelledDomainEvent.Origin` era
`BillActionOrigin`, tipado em 2026-09-08 junto com a trilha do boleto. O dreno grava o evento em
JSON e o processador o reconstrói — e `Enumeration` não tem construtor que o desserializador saiba
usar:

```
Outbox message … (PaymentOrderCancelledDomainEvent) failed on attempt 1; will retry.
System.NotSupportedException: Deserialization of types without a parameterless constructor…
Type 'BillPayment.Domain.Bills.BillActionOrigin'. Path: $.Origin
```

**Todo** cancelamento de ordem morria na fila e ia para dead-letter em cinco tentativas: o espelho
nunca disparava, e o boleto ficava com a data e o vínculo de um pagamento que não ia acontecer.
`Origin` passou a viajar como o **`Name`** (o consumidor já falava nome — a tradução acontece num
lugar só, no `MarkBillScheduleCancelledCommandHandler`).

**A causa raiz estava gravada e não logada**: `OutboxProcessor.RegisterFailureAsync` persistia a
exceção na coluna `error` e passava só a mensagem ao `LogWarning`. Os dois `LogWarning` agora
recebem o `failure` como primeiro argumento — foi essa linha que revelou o resto.

**Guarda nova:** `UnitTests/SeedWork/DomainEventSerializationErosionTests` (3) varre todo
`IDomainEvent` do Domain, reprova propriedade que carregue `Enumeration`, e pede ao próprio
desserializador que construa cada tipo que viaja. Irmã do `DomainEventDrainErosionTests`.

### 🐛 A reversão de decisão recusava com o erro errado (1 falha)

`UndoBillDecisionCommandHandler` roda duas pré-condições que exigem consulta, e a primeira
pergunta se a chave natural foi reocupada. Num boleto que **ainda espera decisão**, quem ocupa a
chave é ele mesmo: a resposta era `BLP.BIL02` ("boleto já capturado"), mandando procurar uma
duplicata inexistente, no lugar do `BLP.BIL38` que explica a recusa. As duas pré-condições passaram
a rodar só quando `bill.AcceptsDecisionUndo` — que é também não consultar o banco à toa num pedido
que o agregado vai recusar. Nenhum bloqueio legítimo mudou: `Denied` e `Cancelled` liberam a chave.

### As 5 restantes: suíte descrevendo um mundo que os ADRs substituíram

- **Webhook (3).** O ADR-019 fez o handler **reler a ordem no provedor** — o corpo virou aviso —, e
  três testes mandavam `BILL_BANK_PROCESSING` no payload sem armar o `ScriptedGet`. Armado.
- **Rota sem tenant (1).** `POST /webhooks/asaas` não casa rota nenhuma, e desde a fase 8 o
  fallback de autorização é aplicado **também quando nenhum endpoint casa** — 401, não 404. O 401 é
  o desfecho melhor: não confirma se a rota existe. A expectativa mudou, a produção não.
- **Cancelar agendamento (1).** O teste esperava `Cancelled`; o ADR-018 devolve a **`Approved`** —
  a aprovação humana sobrevive ao cancelamento do agendamento. Foi a regressão que aquele ADR
  existiu para consertar.

### Verificação

**796 de integração e 1.318 unitários verdes** (`dotnet test`, 2026-09-09), `dotnet build
BillPayment.sln -p:TreatWarningsAsErrors=true` limpo. As duas guardas novas foram provadas pelo
avesso — reintroduzir o defeito deixa cada uma vermelha. Três entradas no `gotchas.md`: o Smart
Enum no evento, a suíte vermelha herdada, e o `[ProtectedResource]` decorativo.
...
- **O checksum de requisição do AWSSDK 4 fica DESLIGADO, e sem isso o Garage recusa toda gravação.** O SDK 4 mudou o padrão para `RequestChecksumCalculation = WHEN_SUPPORTED`, que manda o corpo do `PutObject` em `aws-chunked` com trailer de CRC32; o Garage responde **`400 Bad request: Invalid payload signature`**. `AddAttachmentStorage` força `WHEN_REQUIRED` nos dois sentidos (`ResponseChecksumValidation` junto, senão a leitura do documento original quebra do mesmo jeito). **O erro engana e custou três rodadas de diagnóstico em 2026-09-05**: `Invalid payload signature` parece credencial errada e não é — a assinatura do **cabeçalho** passou, e o que o servidor reprova é a do **corpo**. A escada de erros distingue os três casos, e vale decorar: `Bucket not found` = balde inexistente; **`403 Forbidden: Invalid signature`** = HMAC do cabeçalho, ou seja, segredo errado; **`400 Bad request: Invalid payload signature`** = HMAC do corpo, ou seja, formato que o servidor não aceita. O `PeopleManagement` grava no MESMO balde e nunca viu isto porque está no `AWSSDK.S3 3.7.x` — **ao subir a versão do SDK dele, esta configuração vai junto**. Guarda de erosão em `Storage/S3ClientConfigurationTests`, que monta o contêiner a partir do `AddInfraDependencies` pelo mesmo motivo do `AsaasClientConfigurationTests`: o resto da suíte troca o `IAttachmentStorage` por dublê e é cego para o que o `AmazonS3Config` carrega.
...
- **O INSTRUMENTO e o PAGADOR podem vir de degraus diferentes, e a fusão é aditiva.** Até 2026-09-11 o pagador lido pela visão era descartado sempre que a cascata determinística tivesse resolvido — o `return` antecipado de `ExtractWithVisionAsync` devolvia antes de fundir, e só o retrato de enriquecimento sobrevivia. A **guia do FGTS Digital** é o caso que expôs isso, e a forma não é exótica: o QR Pix resolve o instrumento e a camada de texto do PDF é **ilegível por construção** (fontes subset `Identity-H` sem `/ToUnicode`, e os TrueType embutidos sem `cmap` — não existe mapa glifo→caractere no arquivo, então nenhum extrator de texto resolve), o `TaxIdScanner` volta vazio, e a escada caía em `Unrouted` com o CNPJ do tenant impresso na página. `ExtractionResult.WithParties` acrescenta sem substituir — o que a varredura determinística leu **vem primeiro**, inclusive o que veio sob rótulo de pagador, que é o que sustenta o degrau 1 negativo — e deduplica por `(documento, rótulo)`, a mesma chave do `TaxIdScanner`. **Fundir não reescreve `Method`/`UnlockedBy`**: o degrau que resolveu é métrica da cascata. A segurança é a mesma de sempre: a visão entra `underPayerLabel: false`, então ela só promove e nunca declara "é de outra pessoa".
- **A GFD imprime só a RAIZ do CNPJ, e nada no caminho determinístico a alcança.** `TaxIdScanner` emite sequência de exatamente 11 ou 14 dígitos (ou o documento cadastrado dentro de uma maior), `PartyCandidate.TryCreate` exige DV, e `PayerProfile.OwnsByCnpjRoot` recebe um `TaxId` completo — raiz solta não tem representação. Na GFD isso não importa, porque o texto não entrega dígito nenhum; **passa a importar no dia em que a visão estiver desligada ou a cota esgotada**. Quem fecha a lacuna hoje são os `ExtractionHints`, que levam ao modelo os documentos do próprio tenant.
...
- **Retrato que chega DEPOIS da decisão não revalida — e essa é a guarda mais importante da fila.** `AcceptsValidation` inclui `Approved`, e revalidar ali **pode** derrubar a aprovação (desde 2026-09-10 só quando algo muda — mas um retrato atrasado é exatamente o que muda o desfecho de `DocumentConsistency`): um enriquecimento de fundo desfazendo em silêncio a decisão de uma pessoa é a pior troca possível. `Bill.AcceptsSilentRevalidation` restringe a revalidação a `Captured`/`AwaitingApproval`; fora disso o retrato é anexado (é metadado, e melhora o histórico) e o boleto fica marcado com `ReadingArrivedAfterDecision`.
...
- **A prévia do seletor livre responde o MESMO veredito das quatro sugestões, e por isso pode dizer "não dá".** `SchedulePreviewDto.For` consulta `CanScheduleForToday` quando — e só quando — a data pedida é hoje; fora disso a janela não tem o que dizer. Até 2026-09-10 ela chamava apenas `Resolve`, que não conhece a hora, e prometia para hoje uma data que a escrita recusa com `BLP.BIL40`: a folha escondia a sugestão "pagar hoje" e o seletor livre a devolvia por baixo. **A regra do cliente tem duas metades e as duas importam**: prévia **ausente** não bloqueia (falhar ao buscá-la é rede, não recusa), prévia com `available: false` bloqueia. E o campo ausente lê como `true` — servidor mais antigo continua recusando na escrita, que é onde a guarda de verdade mora.
- **A janela de submissão vale para o pagamento que EXECUTA HOJE, não para a fila inteira — e o recorte mora na REIVINDICAÇÃO.** Até 2026-09-10 o ciclo abortava fora do horário e a ordem de amanhã dormia até as 9h **dentro de casa**, onde uma queda nossa a perde; submetida, ela vira agendamento no provedor, que sobrevive à queda (ADR-021, "Decisões posteriores"). Com a janela fechada o lote traz só o que não executa hoje: **data pedida depois de hoje E boleto não vencido**. As duas metades são necessárias — o provedor processa conta **vencida na hora, ignorando a data** (ADR-017, regra 3), então um vencido pedido para amanhã é pagamento de hoje e continua esperando a abertura. É por isso que a consulta do claim lê `bills.due_date`: é o único ponto em que o vencimento pode ser consultado **a tempo de não reivindicar** — a tentativa é contada na SAÍDA da fila, e reivindicar o que não seria submetido gastaria tentativa e levaria a ordem à desistência sem ela nunca ter falado com o provedor. **Não mova essa decisão para o `SubmitPaymentOrderCommand`.** O `FOR UPDATE` continua sendo só de `payment_orders` (o boleto entra por `NOT EXISTS`, que não trava linha), e boleto ausente não impede a reivindicação: quem recusa ordem órfã é a submissão, com falha visível.
...
- **A permissão é buscada UMA VEZ POR TOKEN, não por requisição** (2026-09-04). `AuthorizationServerClient` tem UM método — `FetchAllPermissionsAsync` — que pede o retrato inteiro ao Keycloak (`response_mode=permissions` **sem** o parâmetro `permission`; é a ausência dele que faz o servidor avaliar tudo). O `RptCache` guarda o retrato chaveado pelo **SHA-256 do token**, nunca pelo `sub`: dois tokens da mesma pessoa podem carregar escopos diferentes, e chavear por identidade faria um herdar as permissões do outro. O que entra no cache é a **`Task`**, não o valor — requisições simultâneas com o mesmo token compartilham UMA busca, senão uma tela que dispara seis chamadas ao abrir produziria seis idas ao Keycloak. **Isso eliminou o segundo round-trip do `approve`**: a alçada de risco (`ResolveRiskClearanceAsync`) lê o mesmo retrato, e o `GetGrantedScopesAsync` deixou de existir. TTL = menor entre `Keycloak:RptCacheTtl` (60 s) e o que resta do `exp`.
- **Falha não é cacheada, mas o retrato anterior sobrevive a ela** (*fail-static*, `Keycloak:RptStaleGrace` = 10 min): Keycloak fora do ar deixa de derrubar quem já estava usando, com log em `Warning`. **Token RECUSADO nunca é servido por retrato velho** — um token revogado tem que parar de valer na hora, e é a única negativa que o cliente resolve sozinho. **Carência zero desliga o mecanismo** e a indisponibilidade volta a ser 503 imediato; guardar com expiração zero não é "guardar por zero tempo", o `MemoryCache` recusa o valor. E **403 do Keycloak é retrato VAZIO, não indisponibilidade**: tratá-lo como falha faria quem não tem permissão receber 503 e, com fail-static, herdar o retrato de outra sessão. Já um **200 com corpo que não é a lista** é indisponibilidade, nunca vazio — negar todo mundo em silêncio porque o provedor mudou de contrato é a pior leitura possível.
- **A suíte roda com `Keycloak:RptCacheEnabled=false`**, e não por conveniência: os testes de alçada variam os escopos pelo header `bp_scopes` mantendo o MESMO token, e o cache — chaveado pelo token, como tem que ser — devolveria à segunda requisição o retrato da primeira. Em produção isso não acontece: permissão vem da identidade do token, não de um header. Quem prova o cache é `Authorization/RptCacheTests`, que troca o token de propósito.
- **O tipo do claim casa por igualdade EXATA desde 2026-09-04**, não `Contains`. Com `Contains`, `"bp_tenants".Contains("tenants")` é verdadeiro: uma API configurada para ler o `tenants` genérico aceitaria também o claim de outro produto. O sentido que nos protegia era acidente de nomenclatura, não desenho.
- ⚠️ **As convenções de nome (client, papel, recurso, escopo, seção de `appsettings`) são normativas e vivem em [`utils/KeyCloakConfig/CONVENCOES.md`](../../../utils/KeyCloakConfig/CONVENCOES.md).** Leia antes de criar papel ou seção nova.
- **`Authorization/RealmContractTests` liga o código ao realm**: todo par `(recurso, escopo)` de um `[ProtectedResource]` tem que existir no `bill-payment-authz-config.json` versionado, e todo recurso do arquivo tem que ser usado por algum endpoint. Recurso que o código pede e o realm não tem faz **todo mundo, inclusive o administrador**, tomar 403 naquele endpoint — e nada no build percebia. O arquivo versionado é a **entrada** do partial import, não a documentação dele.
- **Segregação de funções (2026-09-04): `bill-admin` NÃO aprova.** Ele saiu da `Bill Approver Policy` — um administrador que cadastra o beneficiário, importa o boleto e o aprova sozinho é a falha clássica. Quem precisa das duas coisas recebe os DOIS papéis, e isso fica visível na auditoria em vez de ser efeito colateral do papel de configuração. Pelo mesmo motivo, `capture-source:manage` (credencial do Entra ID do cliente — quem a tem lê a correspondência da empresa) e `payer-profile:manage` (a chave Asaas, que PAGA CONTAS) saíram do `bill-operator` e viraram a `Bill Credential Permission`, só do admin; ao operador sobrou `capture-source:sync`, que é sincronizar caixa já conectada.
- **A alçada de risco virou papel, não configuração de console**: `bill-approver-attention` ⊂ `bill-approver-danger` ⊂ `bill-approver-extreme`, **compostos** no realm. O código já é hierárquico (`RiskLevel.IsCoveredBy`), então o composto é redundante funcionalmente — ele existe para o console listar a cobertura real em vez de exigir que quem atribui saiba a hierarquia de cabeça, e para "quem pode aprovar Extremo Perigo?" ser uma consulta de uma linha.
...
- **O documento é RELIDO a cada validação, e é a releitura que decide a verificação 8** (2026-09-15). `IBillDocumentSource` abre o arquivo guardado e roda a cascata **determinística** — 33 ms medidos, sem IA e sem custo de provedor —, e o resultado entra pelo `BillValidationContext` como as consultas oficiais entram. **`Bill.ExtractedPayer` virou cache da última leitura**, não retrato da captura. As duas metades da regra importam: releitura que devolve "nenhum pagador" é **afirmação** e sobrepõe o que estava guardado; releitura que **não aconteceu** — boleto sem arquivo, balde fora, PDF que nenhuma senha abre — é ausência de resposta e **não apaga nada**. É o que faz "Revalidar" consertar leitura errada, e o que permite fonte nova de boleto precisar apenas **guardar os bytes e a origem**: enquanto o campo vinha congelado, cada caminho de entrada tinha de lembrar de preenchê-lo, e o esquecimento de um deles foi o defeito do RUF101. ⚠️ **Falha fechada**: erro ao abrir o documento nunca derruba a apuração das catorze. **As verificações 11 e 13 continuam lendo o agregado, de propósito** — a 13 porque a leitura por IA custa e tem teto por tenant, a 11 porque o degrau que atribuiu o boleto é fato histórico, como o remetente.
...
- **A varredura de revalidação é o par obrigatório dessa decisão.** Sem ela o "revalide mais tarde" é trabalho manual que ninguém faz. Quatro regras: só `AcceptsSilentRevalidation` (quando a consulta volta a responder, o desfecho do `LookupAvailability` muda e a aprovação cai — worker não desfaz decisão humana, nem sob a regra nova de 2026-09-10); só `lookup_unavailable`; **aborto precoce** do ciclo depois de N indisponibilidades, porque o disjuntor do cliente de consulta é compartilhado com as validações interativas; e **sem teto de tentativas**, ao contrário da fila de leitura — desistir deixaria o boleto em Extremo Perigo para sempre por uma queda que passou. **ADO direto**, tentativa contada na saída, e `updated_at` intocado (lição do `LastSweptAt`).
...
- **Dia de calendário trafega como `DateOnly`; instante, como `DateTime` em UTC — e isto é contrato, não estilo.** Até 2026-09-10 o `BillQueries.ToDateTime` convertia todo dia para meia-noite UTC, e o cliente, que formata em hora local, mostrava o **dia anterior** em qualquer fuso a oeste de Greenwich: vencimento 25/06 aparecia 24/06 na lista e na consulta oficial, e **certo** na evidência do check de prazo — que é texto montado no servidor e por isso escapava. Dois sintomas diagnósticos do mesmo defeito: a data divergindo entre duas telas do mesmo boleto (a lista mandava `ScheduledFor` como `DateOnly` e o detalhe o convertia), e a evidência discordando do campo. Os sete campos de dia do `BillDtos` passaram a `DateOnly?` e o helper foi apagado; `ExpiresAt` do Pix **continua** instante, porque o QR expira na hora. Os demais read models do BC (expectativas, ordens, prévia e opções de agendamento, fontes de captura) sempre usaram `DateOnly` — este era o destoante. Travado por teste que olha o JSON CRU (`BillDueDateProjectionTests`): desserializar em `DateOnly` esconderia exatamente o que o teste existe para pegar.
- **O prazo de validade do retrato é resolvido no servidor e viaja pronto** (`BillDetailDto.SnapshotExpiresAt`, 2026-09-10). O cliente replicava as 12h numa constante própria, e bastava mudar `Approval:MaxSnapshotAgeHours` para a tela passar a mentir — habilitando "Agendar…" num retrato que o servidor ia recusar com `BLP.BIL06`, ou o contrário. `BillQueries` lê a MESMA `ApprovalOptions` que os dois comandos consomem; nenhuma regra nova mora na query.
- **`RecordChecks` exige o catálogo completo** (`BLP.BIL19`) e é o **único** ponto que muda status por validação. Devolve `ValidationOutcome` — o handler nunca lê `bill.Checks` para montar resposta.
- **Revalidar um boleto aprovado só derruba a aprovação quando algo muda** (2026-09-10, decisão do usuário). "Algo" são DUAS coisas: o desfecho de qualquer das catorze verificações (comparado por `Outcome` + `Severity` + `ReasonCode` — **nunca** pela `Evidence`, que carrega valores e datas e mudaria em toda rodada, zerando a regra) **e** o valor a pagar, confrontado com `ApprovalRecord.AmountAtDecision` (coluna nova, migração `ApprovalAmountAtDecision`). A segunda metade não é zelo: `AmountMatch` compara contra a política do beneficiário, não contra o número que o aprovador viu, então num boleto vencido as catorze saem idênticas enquanto o valor sobe — e "nada mudou" autorizaria um débito maior que o consentido. Aprovação legada sem valor gravado cai uma vez para a fila; sem saber contra o que a pessoa consentiu, manter seria presumir. `ValidationOutcome.ApprovalPreserved` diz qual dos dois desfechos ocorreu, porque deduzi-lo do status exigiria saber o estado anterior à chamada.
- **Era incondicional até então, e o efeito era um laço sem saída.** O agendamento reconfere o frescor do retrato (ADR-018), revalidar era o único jeito de renová-lo, e revalidar custava a aprovação — agendar um boleto aprovado no dia anterior exigia aprovar de novo, todas as vezes. Junto veio `BLP.BIL41`: boleto com data escolhida **não** revalida, porque antes ele caía para `AwaitingApproval` sem perder a data (aguardando aprovação com ordem em voo).
...
│   ├── Queries/ArtifactDownload.cs   #   o documento pronto para servir — tipo de mídia (medido nos bytes) e nome resolvidos num lugar só
│   ├── Queries/PrefixedStream.cs     #   devolve ao fluxo do balde os bytes espiados para reconhecer o tipo — o S3 não volta atrás
...
│   ├── Ports/                        #   IBankDirectory, IBillLookupService, IPixLookupService, ISecretVault, IMailboxReader, IBoletoDocumentParser, IAttachmentStorage, IDocumentIntelligence, IDocumentLinkResolver, INotificationSender, IPdfComposer — contratos de mundo externo (não SeedWork)
...
- [ ] **Importar o `bill-payment-authz-config.json` no realm (partial import)** — em 2026-09-04 ele ganhou os escopos de alçada (`approve-attention`/`-danger`/`-extreme`) com **papéis próprios e compostos** (`bill-approver-attention` ⊂ `-danger` ⊂ `-extreme`), a `Bill Credential Permission` (capture-source/payer-profile `manage` só para o admin) e a saída do `bill-admin` da política de aprovação. **Sem o import, ninguém aprova acima de Verde** (403 `BLP.BIL32`) e o operador continua com as duas credenciais. Não é mais preciso montar papel→escopo no console: está tudo no arquivo. — `bill:approve-attention`/`approve-danger`/`approve-extreme` entraram em 2026-08-31 no `bill-payment-authz-config.json`, com UMA permissão default (`Bill Approve Any Risk` → Bill Admin Policy). Sem o partial import, ninguém aprova acima de Verde (403 `BLP.BIL32`); as demais combinações papel→escopo são montadas no console (decisão do usuário — a alçada é hierárquica no código).
...
### Auditoria da ingestão por e-mail (2026-09-03) — resolver no futuro
...
- [x] **Rate limiting** — feito (`Extension/RateLimitingExtensions`, seção `RateLimiting`).
