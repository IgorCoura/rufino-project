...
- **Application**: `RegisterBillExpectation`, `AlterBillExpectationWatch`, `WaiveExpectationCycle`, `SweepBillExpectation` (o job), `FulfillExpectationForBill`, `LearnBillExpectations`, mais cinco handlers de Domain Event e `IBillExpectationQueries` com o **painel de pendências**.
- **API**: `ExpectationsController` (6 endpoints) e `ExpectationSweepBackgroundService` — o **único worker ligado por padrão** além do outbox.
...
**Ficou de fora, de propósito:** o adapter de e-mail. O aviso vai para o log estruturado e o alerta fica **registrado no agregado**, visível em `GET /expectations/pending` — que é o canal que funciona sem configuração nenhuma. Entra no checklist pré-produção.
...
│   └── Expectations/                 #   BillExpectation, ExpectationCycle, Recurrence, CycleStatus, MissReason, AlertLevel, AlertRecord, eventos, IBillExpectationRepository
...
│   ├── Notifications/                #   LoggingNotificationSender (o registro vive no agregado; o canal é substituível)
...
├── BillPayment.UnitTests/            # xUnit — 889 testes; pasta por Aggregate, cada uma com Mothers/
├── BillPayment.IntegrationTests/     # xUnit + Testcontainers + Respawn — 338 testes
│   ├── Infrastructure/               #   IntegrationTestWebAppFactory (Outbox:Enabled=false), BaseIntegrationTest, IntegrationTestCollection, RecordingLoggerProvider
...
- [ ] **Validação do TenantId contra JWT** — hoje `{tenantId}` vem da rota sem validação. Implementar `TenantAuthorizationFilter` que compara o `{tenantId}` da rota com o claim **`tenants`** do token. **O claim se chama `tenants`, não `tenant_ids`**: o `RouteAccessRequirementHandler` (herdado do PeopleManagement) casa claims por `Contains`, e `"tenant_ids".Contains("tenants")` é falso — o guard reprovaria todo mundo com 403. Quem emite esse claim é o BC **TenantManagement** (`server/Services/TenantManagement/`), que passou a existir em 2026-08-13; ver o ADR-003 dele. **Pré-requisito**: rodar o backfill de tenants antes de ligar a validação, senão os `TenantId` já em uso não estarão no claim de ninguém.
...
- [ ] **Adapter de e-mail para `INotificationSender`** — hoje entra o `LoggingNotificationSender`, que registra o aviso no log e nada mais. O alerta continua existindo no agregado e visível em `GET /expectations/pending`, então a rede de segurança **funciona** — mas só para quem abre o painel, e o ADR-014 existe justamente porque ninguém abre painel todo dia. É o item que converte a 2.7 de "registrada" em "avisada". O segredo do SMTP vai por variável de ambiente, nunca no `appsettings.json`.
