...
/// pessoa podem carregar escopos diferentes (papel revogado entre um login e outro), e chavear por
/// identidade faria um herdar as permissões do outro. O hash existe para o token cru não ficar
/// morando em chave de dicionário, onde qualquer despejo de memória o exporia.
/// </para>
/// <para>
/// <strong>O TTL nunca ultrapassa a validade do token.</strong> É o teto natural: passado o
/// <c>exp</c>, a autenticação já recusa a requisição antes de a autorização ser consultada, então
/// guardar além disso só ocuparia memória. Dentro dele vale o
/// <see cref="AuthorizationOptions.RptCacheTtl"/>, que é a janela em que uma permissão revogada no
/// console ainda vale — 60 s por padrão.
/// </para>
/// <para>
...
/// sistema. Passada a carência, volta a ser 503 — degradação silenciosa e eterna seria pior que a
/// queda.
/// </para>
/// <para>
/// Requisições simultâneas com o mesmo token compartilham UMA busca: o que entra no cache é a
/// <see cref="Task{TResult}"/>, não o valor. Sem isso, uma tela que dispara seis chamadas ao abrir
/// produziria seis idas ao Keycloak — exatamente o que este cache existe para evitar.
/// </para>
/// </remarks>
    ...
    ILogger<RptCache> logger) : IRptCache
{
    ...
    public async Task<RptFetchResult> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        // Guarda a TAREFA: as requisições que chegarem enquanto esta ainda corre esperam por ela,
        // em vez de abrirem cada uma a sua ida ao servidor de autorização.
...
#pragma warning restore CS4014
    ...
    {
        // Token recusado NÃO é servido por retrato velho: um token revogado tem que parar de valer
        // na hora, e é a única negativa que o cliente resolve sozinho (renovando).
