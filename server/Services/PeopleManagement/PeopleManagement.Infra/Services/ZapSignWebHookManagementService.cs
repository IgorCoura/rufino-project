    ...
    public class ZapSignWebHookManagementService : IWebHookManagementService
    {
        ...
        public async Task RefreshWebHookEvent(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                // 1. CRIA O NOVO PRIMEIRO (garante que sempre haver� um webhook)
                ...
                if (string.IsNullOrEmpty(newWebHookId))
                {
                    ...
                    return; // Mant�m o webhook antigo ativo
                }
                ...
                try
                {
                    ...
                    // 3. S� DELETA O ANTIGO AP�S CONFIRMAR PERSIST�NCIA
                    if (!string.IsNullOrEmpty(oldWebHookId) && oldWebHookId != newWebHookId)
                    {
                        ...
                        if (!deleted)
                        {
                            ...
                            // N�o � cr�tico - o webhook antigo vai expirar naturalmente
                        }
                    }
                ...
                catch (Exception saveEx)
                {
                    ...
                    // ROLLBACK: Deleta o webhook rec�m-criado
                    await DeleteWebHook(newWebHookId, cancellationToken);

                    // N�o propaga a exce��o - o webhook antigo continua ativo
