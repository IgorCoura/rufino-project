...
/// <c>PeopleManagementWebApplicationFactory</c>), então esta classe é o único lugar que o exercita.
/// </remarks>
public sealed class RptCacheTests
{
    ...
    private static readonly RptFetchResult DocumentApprove =
        RptFetchResult.Resolved(RptSnapshot.From([("document", ApproveScopes)]));
    ...
    {
        var client = new CountingClient(DocumentApprove);
        var (cache, accessor, _) = Build(client);
        UseToken(accessor, "token-a");

        for (var i = 0; i < 5; i++)
            Assert.True((await cache.GetAsync()).Snapshot.Grants("document#approve", ScopesValidationMode.AllOf));
    ...
    public async Task TokenDiferente_NaoHerdaORetratoDoOutro()
    {
        var client = new CountingClient(
            DocumentApprove,
            RptFetchResult.Resolved(RptSnapshot.From([("document", ViewScopes)])));
        var (cache, accessor, _) = Build(client);
        ...
        Assert.False(second.Snapshot.Grants("document#approve", ScopesValidationMode.AllOf));
        Assert.True(second.Snapshot.Grants("document#view", ScopesValidationMode.AllOf));
    }
    ...
    {
        var client = new CountingClient(DocumentApprove, RptFetchResult.Unavailable());
        ...
        Assert.True(afterOutage.Snapshot.Grants("document#approve", ScopesValidationMode.AllOf));
        Assert.Equal(2, client.CallCount);
    }
    ...
    {
        var client = new CountingClient(DocumentApprove, RptFetchResult.InvalidToken());
        var (cache, accessor, _) = Build(client, new AuthorizationOptions
        {
            RptCacheTtl = TimeSpan.FromMilliseconds(1),
            RptStaleGrace = TimeSpan.FromMinutes(10),
