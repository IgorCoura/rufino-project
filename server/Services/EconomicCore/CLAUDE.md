...
Walking Skeleton do aluguel pós-pago: **completo + ciclo de vida do contrato** (Draft → Active → Terminated, com vínculo a Resource/Agent persistidos via API). **Estendido com variações de valor de aluguel** (ver seção "Variações de valor de aluguel" abaixo): encargos multi-trilha (condomínio/IPTU/seguro), pagamento bundled (um boleto → N commitments), juros/multa de atraso **com política configurável por contrato** (% ou valor fixo, acúmulo dia/mês/ano, obrigatória na criação e alterável em Draft/Active via `POST /penalty`), **pagamento one-shot em atraso (boleto único de valor atualizado)** e reajuste mid-term.

| Camada | Status | Artefatos |
|---|---|---|
| Domain | ✅ | 4 Aggregates (EconomicEvent, EconomicResource, EconomicAgent, EconomicContract). `EconomicContract`: `PrimaryPurpose` (trilha-núcleo: Rent/Insurance/PropertyTax) + coleção `Charges` (`ContractCharge`) + `AddCharge`/`Activate` gerando 1 par recíproco **por trilha por período** + `Terminate`/`EnsurePayable`/`EnsureOccupiable`/`TryRegisterLatePenalty` (multa+juros, decide direção/idempotência internamente) + `MaterializeLatePaymentPenalty` (one-shot: materializa/reusa a Penalty sincronamente, valida total = base+penalidade, retorna `LatePaymentSettlement`) + `TrySettlePenaltyCoverage` (perna Penalty auto-liquidante) + `ApplyAdjustmentToAmount`/`ApplyAdjustmentByRate` (reajuste, precificação no agregado, trava período cumprido) + `PenaltyPolicy` (`PenaltyTerms` configurável: fine/interest cada um `PenaltyValueKind` Percent|Fixed + `InterestAccrualPeriod` Daily|Monthly|Yearly, obrigatório no `Create`) + `ChangePenaltyPolicy` (Draft|Active, emite `ContractPenaltyTermsChanged`). `EconomicEvent`: coleções `Allocations` (`PaymentAllocation`) + `DualityLinks` (`DualityLink` com `CommitmentId`, achatado em escalares) + `RegisterBundledPayment` (1 caixa → N alocações) + `RegisterPaymentCoverage`/`RegisterConsumptionCoverage` (1:1) + `CloseDuality(commitmentId,…)` por alocação + `CloseDualityAsSelfSettled` (fecha a perna contra o próprio evento — trilha Penalty). Acessores p/ orquestração humble: `GetInflowCommitmentIds` + `ResolveBundledCompetence` (política de competência do bundled no agregado); `Terminate` retorna o count da cascata. Smart Enums `CommitmentPurpose`, `PenaltyValueKind`, `InterestAccrualPeriod` (dono do `ElapsedUnits`). DualityMatchingService (`Match` por par de commitments). 411 unit tests |
| Application | ✅ | **Mediator próprio (sem MediatR)**. 12 Commands (RegisterEconomicResource, RegisterEconomicAgent, RegisterEconomicContract **com `Charges`+`PrimaryPurpose`+`PenaltyTerms` obrigatório (CTR50 se ausente)**, ActivateEconomicContract, TerminateEconomicContract, GenerateCommitments, RegisterConsumptionEvent, RegisterPaymentEvent, **RegisterBundledPaymentEvent**, **RegisterLatePaymentEvent** (one-shot, único `IMultiAggregateCommand`), **ApplyContractAdjustment**, **ChangeContractPenaltyTerms**) — todos em `IdentifiedCommand`. Query side via **`IReportQueries`/`ReportQueries`** (GetCompetenceDRE, GetCashFlow) chamado direto pelo controller, **fora do mediator** (padrão eShop `IOrderQueries`). Handlers mutam **um único agregado** (exceção sancionada: `RegisterLatePaymentEvent`, marcado com `IMultiAggregateCommand`); duality-close + late-penalty acontecem no handler async de Outbox |
| Infra | ✅ | EconomicCoreDbContext (UoW + Outbox + `ClientRequests`), 4 repositories (`FindCoveredByCommitmentAsync`/`GetLastInflowPeriodForCommitmentsAsync` agora via coleção `Allocations`), EF mappings: contract com `primary_purpose` + penalty achatado em 5 colunas escalares (`penalty_fine_kind`/`penalty_fine_value`/`penalty_interest_kind`/`penalty_interest_value`/`penalty_interest_period`, kinds/period como Smart Enum Id) + tabela `contract_charges` (OwnsMany) + `commitments.purpose`; event com tabelas `economic_event_allocations` + `economic_event_duality_links` (OwnsMany). **Outbox consumer** + `CloseDualityOnEconomicEventRegisteredHandler` (fecha N dualidades por covering + **gera trilha Penalty automaticamente em pagamento tardio** + **self-settle da perna Penalty** via `TrySettlePenaltyCoverage`/`CloseDualityAsSelfSettled`), `RequestManager` |
| API | ✅ | ResourcesController, AgentsController, ContractsController (+activate +terminate +**adjust** +**penalty**), EventsController (+payment +**payment/bundled** +**payment/late**), ReportsController, DomainExceptionFilter. Writes em `IdentifiedCommand` com header `x-requestid`. **Breaking change**: `POST /contracts` agora exige o bloco `penalty` no body |
| Integration Tests | ✅ | 79 tests: jornada 12 meses, **política de penalidade (criação sem penalty → CTR50, `POST /penalty` em Draft/Active, Terminated → CTR51, idempotência, penalidade pré-materializada não recalcula, juros diário two-step, política fixa one-shot)**, **pagamento bundled (1 boleto, 2 alocações, 2 dualidades, 4 commitments)**, **pagamento tardio → trilha Penalty via relay**, **pagamento one-shot em atraso (1 evento, 2 alocações, 4 commitments FULFILLED, perna Penalty self-settled)**, **regressão do two-step penalty payment (penalty paga via /payment fecha a duality por self-settle)**, exception suites, multi-tenant, outbox, idempotência (+ suíte de cenários da Fase 8, incl. S10 reajuste mid-term com 4 atrasos two-step, S11 idem com 4 atrasos one-shot e S12 troca de política fixa→percentual mid-term com reajuste e atrasos one-shot) |

## Architecture — what is non-obvious

Prefixos de erro em uso: `SWK##` (SeedWork), `ECC##` (BC transversal), `ECC.<AGG>##` (Aggregate-specific — siglas reservadas: `AGT`, `RES`, `EVT`, `CTR`, `DMS`, `STD`, `SHK`). Faixas em uso hoje: `ECC.CTR01`–`CTR51` (CTR22–28 = charges multi-trilha; CTR30 = componente percentual de penalidade fora de [0,1]; CTR40–42 = reajuste/adjustment; CTR43 = trilha inexistente p/ valor corrente; CTR44 = commitment já coberto por outro evento — anti-duplicata de cobertura, usado pelos handlers de Payment/Consumption/Bundled/LatePayment; CTR45–47 = pagamento one-shot em atraso: total ≠ base+penalidade, pagamento não-atrasado, alvo na própria trilha Penalty; CTR48 = valor fixo de penalidade negativo; CTR49 = kind/period nulos na composição do `PenaltyTerms`; CTR50 = criação de contrato sem o bloco penalty — lançado pelo handler; CTR51 = alteração de penalty fora de Draft|Active — Conflict 409), `ECC.EVT01`–`EVT18` (EVT16–18 = alocações de pagamento bundled). Convenções herdadas:
...
- **Juros/multa de atraso (`PenaltyTerms` + `TryRegisterLatePenalty`)**: penalidade **nasce no atraso** (Hruby §10.5), nunca pré-gerada. `PenaltyPolicy` é **configurável por contrato e obrigatória na criação** (CTR50 se ausente — defaults 2%/1% foram removidos): multa e juros são cada um `PenaltyValueKind` (`PERCENT` do valor do commitment, range [0,1] CTR30, ou `FIXED` em R$ na currency do commitment, ≥0 CTR48), e o juros acumula por **unidade completa decorrida** de `InterestAccrualPeriod` (`DAILY` = dias exatos, `MONTHLY` = meses-calendário, `YEARLY` = anos-calendário; `ElapsedUnits` vive no Smart Enum). Fórmula no VO: `ComputePenalty(base, dueDate, paidDate)` = multa única + juros×unidades (juros `FIXED` = R$X×unidades). **A decisão de penalizar é toda do agregado**: `TryRegisterLatePenalty(commitmentId, paidDate, Func<CommitmentId>, occurredAt)` checa internamente direção (só outflow não-Penalty), janela (`FulfillmentWindow.To`), idempotência (uma por período). O relay (`CloseDualityOnEconomicEventRegisteredHandler`) apenas chama o método para **cada perna** da duality com a data do respectivo evento — sem raciocínio de domínio na Infra (a perna de inflow é no-op). **A política é alterável em Draft|Active** via `ChangePenaltyPolicy`/`POST {contractId}/penalty` (fora disso CTR51, Conflict); penalidades já materializadas **não são reprecificadas** — a obrigação materializada prevalece (mesma regra do reuso no one-shot). Emite `ContractPenaltyTermsChanged`; o VO é mapeado achatado em 5 colunas escalares (sem Money aninhado — regra do owned de 2º nível).
- **Pagamento one-shot em atraso (`MaterializeLatePaymentPenalty` + `RegisterLatePaymentEvent`)**: o caso real dominante é um boleto único de **valor atualizado** (base + multa/juros). O fluxo reativo (relay gera a Penalty *depois* do pagamento base) não consegue representá-lo — a Penalty precisa existir *antes* do caixa. O comando `POST /events/payment/late` resolve: o agregado **materializa (ou reusa) a trilha Penalty sincronamente** (`MaterializeLatePaymentPenalty` — gates de atraso CTR46, trilha base CTR47, total = base+penalidade CTR45; retorna `LatePaymentSettlement` em primitivos) e o handler registra **um** `RegisterBundledPayment` com 2 alocações (base→Rent, penalidade→Penalty). É o **único command multi-aggregate** (contrato + evento num só `SaveEntitiesAsync`, marker `IMultiAggregateCommand`) — split em dois saves quebraria a idempotência (a marca de `client_requests` comitaria no 1º save; crash entre saves → retry devolveria duplicata neutra sem o evento). **A perna Penalty é auto-liquidante**: não há consumo recíproco (a contrapartida foi a tolerância ao atraso, já consumida), então o relay chama `TrySettlePenaltyCoverage` (agregado cumpre as duas pernas) + `CloseDualityAsSelfSettled` (alocação fecha contra o próprio evento) — sem isso a duality da Penalty paga nunca fecharia (bug latente do fluxo two-step, corrigido junto). Penalty reusada não recalcula atraso: a obrigação já materializada vence. V1 mono-trilha (idempotência da Penalty é por período global).
...
- **`IMultiAggregateCommand` (marker em `Application/Mediator/`)**: exceção sancionada e greppável à regra "um agregado mutado por transação". Não há `TransactionBehavior` associado enquanto todo handler tiver exatamente **um** `SaveEntitiesAsync` (a transação implícita do EF cobre os agregados tracked + outbox + `client_requests` atomicamente); o marker documenta a intenção e é o ponto de extensão se um behavior se tornar necessário. Uso atual: só `RegisterLatePaymentEventCommand`. Toda nova adoção exige justificativa nesta seção.
...
│   ├── Commands/                     #   RegisterEconomicResource, RegisterEconomicAgent, RegisterEconomicContract, ActivateEconomicContract, TerminateEconomicContract, ChangeContractPenaltyTerms, ApplyContractAdjustment, GenerateCommitments, RegisterConsumptionEvent, RegisterPaymentEvent, RegisterBundledPaymentEvent, RegisterLatePaymentEvent (cada Handler.cs traz o par <Cmd>IdentifiedCommandHandler)
...
├── EconomicCore.UnitTests/           # xUnit — 411 domain unit tests (inclui EconomicContractChangePenaltyPolicyTests, PenaltyTermsTests, InterestAccrualPeriodTests)
├── EconomicCore.IntegrationTests/    # xUnit + Testcontainers + Respawn — 78 integration tests (inclui ChangeContractPenaltyTermsTests)
