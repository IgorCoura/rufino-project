...
**Domain: Sprints 0–9 implemented.** Application, Infra and API projects are still scaffolding-only (csproj + `Program.cs`). Next sprint is **Sprint 10 — Auto-approval Policy (alçadas múltiplas)**.
...
Not started yet: Application layer (commands/queries/handlers via custom mediator — pendências críticas: **handler para `InstallmentPlanCancelled` aplicando cancel em cascata** (Sprint 8) e **handler que roda `PayableAutoClassifier` ao consumir `PayableCreatedFromCapture` antes de apresentar pro usuário** (Sprint 9)), Infra (DbContext, repositories, EF mappings, event store para `Payable`, unique index `(TenantId, CapturedBillId)` para dedup do Sprint 7), API (controllers, Keycloak auth, filters), and Sprints 10–11. The `PaymentOrder` Aggregate (Sprint 6) and the `CapturedBill` Aggregate (Sprint 7) themselves live in sibling BCs — this BC only carries weak Id references (`PaymentOrderId`, `CapturedBillId`) and consumes/produces integration events at the seams.
