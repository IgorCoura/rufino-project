...
| 1 — Supplier | ✅ Done (refatorado em 12.0) | `Suppliers/Supplier` + `SupplierBankAccount` **(VO selado polimórfico: `SupplierPixAccount` \| `SupplierBankTransferAccount`)**, VOs (`LegalName`, `TradeName`, `TaxId`, `ContactInfo`, `EmailAddress`, `PhoneNumber`, `Address`, `PixKey`), enums (`SupplierStatus`, `TaxIdType`, `BankAccountType`, `PixKeyType`), `Services/SupplierUniquenessChecker` + `ISupplierTaxIdLookup` port. Coleção dupla `ActiveBankAccounts` / `InactiveBankAccounts` no `Supplier` (soft delete preserva snapshot histórico via VO duplicado em inativos). Eventos: `SupplierBankAccountAdded`/`SupplierBankAccountDeactivated` com payload polimórfico (`Variant` + campos nullables) |
...
| 12 — PaymentMethod + PaymentInstrument na criação do Payable | ✅ Done | **12.0** — `SupplierBankAccount` promovido a VO selado polimórfico (`SupplierPixAccount` \| `SupplierBankTransferAccount`), removendo `SupplierBankAccountId`. Coleção dupla ativos/inativos no `Supplier`. **12.A** — `PaymentMethod` reorganizado para 3 valores (`SupplierTransfer`, `DynamicPix`, `BankSlip` — TED unificado com PIX-cadastro; PSP decide canal). Novos VOs: `EmvPayload` (CRC16-CCITT), `BarcodeDigits` (DV mod-11; método `ToDigitableLine()` deriva a linha digitável), `DigitableLine`, `PaymentInstrument` abstract com 4 variantes seladas (`SupplierPixTransferInstrument`, `SupplierBankTransferInstrument`, `DynamicPixInstrument`, `BankSlipInstrument` — apenas `BarcodeDigits`). Siglas de erros: EMV/BCD/DLN/PIN. **12.B** — `Payable.Initialize*` recebem `PaymentInstrument` na criação; eventos `PayableCreated*` carregam serialização flat do instrumento (helper interno `PaymentInstrumentSerialization` faz `Expand`/`Rebuild`). Erro `AP.PAY18`. **12.C** — `RequestPayment(occurredAt)` simplificado (sem `method`/`bankAccountId`); `PayablePaymentRequested` carrega o instrumento serializado. Domain Service `Services/PaymentInstrumentResolver` valida snapshot×Supplier atual (TenantId/SupplierId + conta ativa). Erro `AP.PAY21`. **12.D** — `InstallmentPlanFactory.Create` aceita `IReadOnlyList<PaymentInstrument>` de tamanho `N` (1 boleto distinto por parcela). Erro `AP.IPL08`. **12.E** — `Payable.FlagInstrumentOutdated(reason, occurredAt)` idempotente (emite uma vez ever); estado `IsInstrumentOutdated`/`OutdatedAt`/`OutdatedReason`; novo evento `PayableInstrumentOutdated`. Domain Service `Services/OutdatedInstrumentDetector` retorna `OutdatedInstrumentReport` (campos divergentes + motivo formatado). Erro `AP.PAY22`. **12.F** — atualização de documentação. **12.G** — refactor pós-12.F: remove `PaymentMethod` redundante (era duplicado com `PaymentInstrument.Method`). Factories do Payable e `InstallmentPlanFactory.Create` perdem o parâmetro; `Payable.PaymentMethod` removido do estado (consumidores acessam via `PaymentInstrument.Method`); `PaymentMethodName` removido dos 4 eventos; `AP.PAY19` (`PaymentMethodInstrumentMismatch`) descartado e marcado reservado (inconsistência impossível por construção) |
...
**`SupplierBankAccount` é VO selado polimórfico, não Entity (Sprint 12.0).** A hierarquia tem 2 variantes mutuamente exclusivas: `SupplierPixAccount(PixKey)` para chave PIX estática, e `SupplierBankTransferAccount(BankCode, Branch, AccountNumber, AccountType)` para conta tradicional. Igualdade estrutural por componentes da variante; instâncias de variantes diferentes nunca são iguais (garantido pelo `GetType()` no `ValueObject.Equals`). O `Supplier` mantém **duas coleções**: `ActiveBankAccounts` (sem duplicatas — invariante) e `InactiveBankAccounts` (pode ter duplicatas — preserva histórico de add+deactivate+add+deactivate). Soft delete via `DeactivateBankAccount(vo, occurredAt)` move o VO para inativos e emite `SupplierBankAccountDeactivated`. Não há `Update` no VO — qualquer alteração de dados bancários é Add+Deactivate.

**`PaymentInstrument` (Sprint 12.A) é VO selado polimórfico em `Payables/ValueObjects/` com 4 variantes**: `SupplierPixTransferInstrument` e `SupplierBankTransferInstrument` (ambos herdam de `SupplierTransferInstrument` abstract — carrega `SupplierLegalName` + `SupplierTaxId` no snapshot), `DynamicPixInstrument(EmvPayload)` (unifica boleto-PIX, copia-e-cola e chave temporária), e `BankSlipInstrument(BarcodeDigits)` — apenas o código de barras como forma canônica. A linha digitável (47 dígitos) é **representação derivada** do barcode (não vive no instrumento), exposta via `BarcodeDigits.ToDigitableLine()`; o VO `DigitableLine` continua existindo como tipo standalone para parse de input do usuário. O `PaymentMethod` enum tem 3 valores (`SupplierTransfer`, `DynamicPix`, `BankSlip`) — TED foi unificado com PIX-cadastro em `SupplierTransfer`: o PSP (Asaas) decide se executa via PIX-DICT ou TED conforme disponibilidade. O design original (Consolidado D-147) coloca esse VO em `PaymentExecution.Domain` — quando esse BC materializar, o VO migra para lá ou para um `Shared.Domain`.

**O instrumento é decidido na criação do Payable, não no `RequestPayment` (Sprint 12.B/12.C, refinado em 12.G).** `Initialize`/`InitializeFromCapture`/`InitializeAsInstallment` recebem **apenas** `PaymentInstrument` — o `PaymentMethod` é **derivado de `instrument.Method`**, não vive como parâmetro separado nem como campo no estado do Payable (consumidores que precisam do canal acessam `payable.PaymentInstrument.Method`). Os eventos `PayableCreated*` carregam serialização flat do instrumento (12 campos extras: `InstrumentKind` + 11 nullables) via helper interno `PaymentInstrumentSerialization`. `RequestPayment(occurredAt)` lê o instrumento do estado e propaga em `PayablePaymentRequested`. Instrumento é imutável após criação — trocar = padrão Substituição (D-134). Slots `AP.PAY19` (PaymentMethodInstrumentMismatch) e `AP.PAY20` ficam reservados — descartados porque a inconsistência se tornou impossível por construção.

**`PaymentInstrumentResolver` (Sprint 12.C) é o gate síncrono cross-aggregate** chamado na Application antes de despachar para o PSP: confere tenant/supplier match + (para variantes `SupplierTransfer`) que o snapshot do instrumento corresponde a uma conta ativa do `Supplier` atual. Erro: `AP.PAY21`. `DynamicPix` e `BankSlip` são self-contained (payload bruto) — Resolver pula a validação.

**`OutdatedInstrumentDetector` (Sprint 12.E) é o proativo (opção B+C).** Detecta divergência snapshot×Supplier atual e retorna `OutdatedInstrumentReport` com lista de campos divergentes. A Application reage a `SupplierBankAccountDeactivated`, roda o detector contra Payables não-terminais do mesmo supplier, e — quando há divergência — chama `Payable.FlagInstrumentOutdated(reason, occurredAt)`. O comando é **idempotente por evento**: emite `PayableInstrumentOutdated` uma única vez ever; chamadas subsequentes são no-op silenciosas. Status do Payable não muda — é uma anotação para notificação ao usuário.
