...
**Domain: Sprints 0–8 implemented.** Application, Infra and API projects are still scaffolding-only (csproj + `Program.cs`). Next sprint is **Sprint 9 — Classificação automática (`ExpenseClassificationRule`)**.
...
Not started yet: Application layer (commands/queries/handlers via custom mediator, **handler para `InstallmentPlanCancelled` aplicando cancel em cascata nas Payables não-terminais — crítico para o Sprint 8 fechar produto**), Infra (DbContext, repositories, EF mappings, event store para `Payable`, unique index `(TenantId, CapturedBillId)` para dedup do Sprint 7), API (controllers, Keycloak auth, filters), and Sprints 9–11. The `PaymentOrder` Aggregate (Sprint 6) and the `CapturedBill` Aggregate (Sprint 7) themselves live in sibling BCs — this BC only carries weak Id references (`PaymentOrderId`, `CapturedBillId`) and consumes/produces integration events at the seams.
