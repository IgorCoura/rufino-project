...
| `Invalid` (4) | Erro ou engano — **sem valor legal nenhum** | `RequiresValidation` (recusa), `OK`/`Pending` (invalidação), pendente duplicada descartada | não |
...
- **`Expired` → `Deprecated` acontece em `SupersedeUnitsCoveredBy`**, disparado por toda entrega (`InsertUnitWithoutRequireValidation`, `MarkAsValidDocumentUnit`, `MarkAsNotApplicableDocumentUnit`). O alcance normal é a competência da unidade que resolveu, **mas toda unidade vencida é depreciada, de qualquer competência**: a renovação de um documento por competência cai na competência SEGUINTE, então exigir competência igual deixaria a vencida esperando um substituto que por construção nunca chega na dela.
...
- **Depreciar e invalidar sempre deixam uma pendente no lugar** (`DocumentService.DeprecateDocumentUnit` / `InvalidateDocumentUnit` → `ReplaceDocumentUnit`). A exigência continua de pé, e como o botão de criar unidade avulsa saiu da tela este é o único caminho para o RH voltar a ter o que preencher — vale também para a recusa de validação. `NewDocumentUnit` reaproveita pendente equivalente, então chamar sempre é idempotente.
- **Endpoints:** `PUT /document/DocumentUnit/deprecate` (`[ProtectedResource("document","deprecate")]` — **scope novo, precisa existir no Keycloak**), `PUT /document/DocumentUnit/invalid` (`reject`), `PUT /document/DocumentUnit/not-applicable` (`mark-not-applicable`). Regra de tela: depreciar só de `OK`; invalidar de `Pending` ou `OK`; não aplicável só de `Pending`. O agregado é mais permissivo no invalidar porque o mesmo caminho serve à recusa de validação.
- **Migration `AddExpiredDocumentUnitStatus`**: coluna `ExpirationCount` + backfill de `Deprecated` sem substituto → `Expired` (o mesmo predicado que o dashboard usava) + backfill do contador. `Invalid` **não** é convertido: entrava no bucket "Vencidos" do dashboard antigo, mas pela definição é erro, não vencimento.
