...
- **Criação manual de uma competência (`POST /document` com `Date`).** O mesmo endpoint da criação avulsa ganhou uma data **opcional**: sem ela, o comportamento de antes, intacto (é o que o app legado usa — a unidade nasce esperando data, na competência mínima quando o template é por competência); com ela, o RH está preenchendo à mão a competência que ficou sem unidade. As regras são todas do servidor, porque a tela enxerga só uma página de unidades:
  - **Só documento por competência aceita** (`PMD.DOC28`). Fora dele duas unidades não podem cobrir ao mesmo tempo, e a próxima nasce de renovar ou de depreciar/invalidar a vigente.
  - **Competência ocupada recusa** (`PMD.DOC27`, `Document.NewDocumentUnitForPeriod`). Ocupa quem não é `Invalid` nem `Deprecated` (`DocumentUnit.OccupiesPeriod`) — pendente, em curso, `NotApplicable` e **vencida** inclusive; as duas que desocupam são exatamente as que já saíram de cena. É a diferença deliberada para `NewDocumentUnit`, que reaproveita a pendente equivalente em silêncio: quem pede criação manual quer uma unidade A MAIS, e devolver a que já existe esconderia o problema. A pendente da competência **mínima** não ocupa competência nenhuma — ela espera data, então não bloqueia.
  - **A unidade nasce pronta**: o serviço chama o mesmo preenchimento do update (`DocumentService.FillUnitDetails`, extraído de `UpdateDocumentUnitDetails` e compartilhado pelos dois), então data, competência, validade e snapshot já vêm gravados. Sem isso o RH teria de reabrir a unidade e digitar a mesma data de novo, e ela nasceria sem validade. Efeito esperado: em template com carga horária a criação passa pelas mesmas guardas de dia útil (`PMD.DOC19`) e conflito de horas (`PMD.DOC10`).
  - **`DocumentDto.PeriodTypeId`** (id do `PeriodType`, null = sem competência) foi ao read model para o cliente saber onde a ação existe. `UsePreviousPeriod` não servia: é falso tanto no documento sem competência quanto no que usa a corrente. Sai de `template.GetPolicy<IPeriodPolicy>()` no `DocumentQueries.GetById` — as policies são `OwnsMany`, então vêm com o template materializado.
  - Escopo: o `create` que o endpoint já tinha. Cobertura em `DocumentPeriodCreationTests` (unitário: cada status que ocupa e os dois que não) e `DocumentPeriodLifecycleTests` (integração: criação, retroatividade, recusa por ocupação, recusa sem competência, e o caminho sem data seguindo reaproveitando).
...
- **O teto do template NUNCA recusa uma ação do RH.** `MaxRenewals` diz quantas vezes o documento **vence**, não quantas vezes o RH pode agir: esgotados os ciclos, renovar / substituir / depreciar / invalidar continuam disponíveis e as unidades novas passam a nascer **sem validade**, exatamente como num template sem regra de vencimento. Enquanto o teto recusava a renovação (`PMD.DOC26`), uma unidade **vencida** com a cota esgotada ficava sem saída nenhuma na tela — não dava para renovar, e vencida também não é invalidável (`PMD.DOC24`, é a prova do período). Chegava-se lá por dado legado (o contador foi backfillado de vencimentos), por edição do teto no template, ou descartando a substituta de uma renovação já feita. **`PMD.DOC26` está aposentado e o código não deve ser reutilizado.**
- **`Document.RenewalCount` conta ciclos de validade GASTOS, não vencimentos** (era `ExpirationCount`; migration `RenameExpirationCountToRenewalCount`, o valor legado é bom porque vencer renovava automaticamente). Só `RegisterRenewal` incrementa; `ExpireDocumentUnit` não mexe mais. Contar vencimento errava dos dois lados depois que renovar virou decisão: renovar antes do prazo nunca vencia a unidade e portanto **nunca consumia cota** — um teto de N ciclos valeria para sempre —, e um documento vencido e abandonado queimava a cota sem nenhuma renovação ter existido. **Renovar com a cota esgotada também não incrementa**: a unidade criada ali nasce sem validade, então não abre ciclo — é o que faz aumentar o teto no template devolver ciclos de verdade, em vez de esbarrar num número inflado. Depreciar/invalidar **não** passam por `RegisterRenewal`: são correção, não troca de ciclo. Documentos legados que já gastaram a cota **não** recuperam ciclos (decisão explícita: sem backfill).
- **`DocumentService.RenewDocumentUnit` é onde a decisão mora** (cruza dois aggregates: cota no template, contador no documento). Ordem: idempotência primeiro (`LiveReplacementFor` devolve a mesma substituta sem consumir ciclo de novo — duplo clique ou retry), depois `NewReplacementUnit`, e `RegisterRenewal` **só quando `IExpirationPolicy.HasValidityCycleLeft(RenewalCount)`** (sem policy de vencimento nunca houve ciclo a gastar, então também não incrementa). A substituta nasce **sem data de referência** (competência mínima, esperando a real) e com a configuração ATUAL do template. A única guarda que resta é de **status**: `DocumentUnit.CanBeRenewed` = `OK`/`Warning`/`Expired` (`PMD.DOC25`).
...
- **Cobertura:** `DocumentUnitExpiredStatusTests` (unitário: contador, vínculo, status com renovação em voo, depreciação da substituída na entrega, idempotência do `LiveReplacementFor`) e `DocumentExpirationRenewalTests` (integração: jobs não criam nada, a regressão do `Include` filtrado, renovação antes/depois do vencimento, idempotência, o ciclo sendo consumido **mesmo renovando sempre antes de vencer** — a regressão que motivou trocar o contador —, a substituta parando de vencer no teto contra o contraponto abaixo dele, e as duas saídas que o teto travava: **renovar uma vencida com os ciclos esgotados** e **renovar de novo depois de descartar a substituta**).
...
- **Expiration is two policies under one capability.** `ExpirationPolicy` expires forever (`HasValidityCycleLeft(_) => true`); `ExpirationLimitedPolicy(Duration, MaxRenewals)` expires `MaxRenewals` times and then stops expiring (`HasValidityCycleLeft(count) => count < MaxRenewals`, onde `count` é `Document.RenewalCount`). Both share `PolicyType.Expiration` — the discriminator between them is `ExpirationParams.MaxRenewals` (`int?`): **null = forever, present = limited**. So rows written before the limited variant (and the backfill) rehydrate as `ExpirationPolicy` with no data migration. `ToPersistence` must match `ExpirationLimitedPolicy` **before** `IExpirationPolicy` — matching by the interface would drop `MaxRenewals`. Limited invariants: `Duration > 0` (`PMD.DOCT11`) and `MaxRenewals >= 1` (`PMD.DOCT12`). **O teto governa VALIDADE, não permissão** (ver "Renovação"): `DocumentService.ValidityDurationFor` é o **único** lugar em que ele age — lê a policy no update e dá à unidade **nenhuma validade quando os ciclos acabaram**, então dali em diante o documento para de vencer e fica OK indefinidamente. `RenewDocumentUnit` consulta a mesma policy só para decidir se conta o ciclo (`RegisterRenewal`), nunca para recusar. No policy ⇒ never expires (back-compat). **The counter is a column on `Document`, not a count of units by status**: any status-based count breaks, because the expired unit becomes `Deprecated` once the replacement arrives and supersession (re-upload/re-validation) also deprecates — correcting a document would consume a cycle.
...
- **O nome tem que continuar `__EFMigrationsHistory`**, o padrão do EF — e **não** o `__ef_migrations_history` que o BillPayment usa. Os ambientes já existentes gravaram o histórico com o nome padrão, então renomear faria o EF não encontrar registro nenhum e tentar reaplicar as 18 migrações num banco que já as tem.
- ⚠️ **Declarar o schema do histórico MOVE a tabela, e nem todo ambiente a tinha onde se supunha.** Este arquivo afirmava que o `search_path` já resolvia o histórico para `people_management` em todos os ambientes existentes; **em produção não resolvia**. A connection string da nuvem não carrega o `SearchPath=people_management` que a local carrega, então até 2026-09-04 o histórico de lá vivia em **`public`**. Quando o `MigrationsHistoryTable(..., DEFAULT_SCHEMA)` entrou, o EF passou a procurar em `people_management`, criou a tabela vazia, concluiu que o banco era virgem e tentou reaplicar a `Init` — morrendo em `42P07 relation "ClientRequests" already exists` em loop de restart. O reparo é mover os registros (`INSERT ... SELECT` de `public` para `people_management`, depois renomear a de origem para `__EFMigrationsHistory_pre_schema_move`); o schema em si estava íntegro e em dia. **Antes de mexer em qualquer coisa que decida ONDE o EF procura o histórico** — schema, nome de tabela ou `SearchPath` da connection string —, consulte `information_schema.tables` em cada ambiente: o local não responde pelos outros, e o sintoma é sempre reaplicar migração em banco cheio.
...
- **Connection string faltando não falha: cai no `appsettings.json` e tenta o host do compose.** As chaves são `ConnectionStrings:PeopleManagement` (era `Postgresql`) e `ConnectionStrings:BackgroundJobs` (era `HangfireConnection`), renomeadas em 2026-09-04. Variável de ambiente com o nome antigo deixou de ser lida **em silêncio**, e o `GetConnectionString` devolveu o default de desenvolvimento — foi assim que a nuvem foi parar num `Name or service not known` para `peoplemanagement.db`, que é nome de serviço do compose e não existe fora dele. Sintoma diagnóstico: **se o log de erro cita `peoplemanagement.db`, a configuração do ambiente não chegou** — nenhum outro arquivo do repositório produz esse host. As demais chaves renomeadas na mesma data estão no `_migracao_2026_09_04` do `appsettings.json` e falham mais tarde, na primeira requisição que usa cada componente.
