...
namespace Security.Jwt.Core.Jwt
{
    internal class JwtService : IJwtService
    {
        ...
        public async Task<ReadOnlyCollection<KeyMaterial>> GetLastKeys(int i)
        {
            var keys = await _store.GetLastKeys(5);
            if (!keys.Any())
            {
                await GenerateKey();
                keys = await _store.GetLastKeys(5);
            }
            return keys;
        }
