...
namespace MaterialPurchase.Tests.Utils
{
    public static class EntitiesExtension
    {
        ...
        {
            var repMaterial = rep.Materials.OrderBy(x => x.Material.Id).ToArray();
            var checkMaterial = req.Materials.OrderBy(x => x.MaterialId).Select((m, index) => m.EqualExtesion(repMaterial[index])).Any(x => x == false);
        ...
        {
            var repMaterial = rep.Materials.OrderBy(x => x.Id).ToArray();
            var checkMaterial = req.Materials.OrderBy(x => x.Id).Select((m, index) => m.EqualExtesion(repMaterial[index])).Any(x => x == false);
