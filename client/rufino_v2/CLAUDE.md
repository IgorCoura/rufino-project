...
- **O refresh é single-flight, e isso NÃO é otimização.** `getCredentials()` é chamado por toda requisição (via `getAuthorizationHeader`), e o splash dispara três `loadPermissions` num `Future.wait` — na virada do token as três entravam juntas no refresh. O realm tem `revokeRefreshToken=true`, então o token rotaciona a cada uso: a segunda chamada apresentava um token já gasto e o Keycloak **revogava a sessão inteira** como replay, jogando o usuário no login em intervalos aparentemente aleatórios. `_refreshInFlight` (nos dois serviços) faz quem chega durante um refresh esperar o mesmo. O `whenComplete` limpa o campo **também no erro** — sem isso uma falha de rede congela o campo e nenhum refresh futuro acontece. O guard é por isolate: **duas abas do navegador ainda colidem**, e quem absorve isso é `refreshTokenMaxReuse=2` no realm. Coberto por teste que conta as chamadas ao token endpoint (`auth_api_service_test.dart`, `auth_code_api_service_test.dart`).
- **Tempos do realm `rufino` (2026-09-04):** access token do `rufino-app` 15 min (override do client sobre os 300s do realm); sessão SSO **24 h idle = 24 h teto** sem "Lembrar de mim" (login diário) e **7 dias idle / 10 anos de teto** com ele marcado. Aumentar o access token não alonga sessão — quem decide isso é a sessão SSO — e só alarga a janela em que uma permissão revogada continua valendo.
...
- **Não existe botão de criar unidade avulsa.** Pendência nasce do evento de admissão, de **Renovar**, ou de depreciar/invalidar a vigente — os caminhos do servidor deixam a substituta no lugar, então a lista recarregada mostra a nova pendente. A única criação manual é a **por competência**, abaixo.
...
## Adicionar unidade por competência (perfil do funcionário)

Dentro do documento expandido, abaixo das unidades, há o botão **Adicionar** (`document-add-unit`) — a criação manual da unidade de uma competência que ficou sem nenhuma. É a exceção à regra de que unidade nasce sozinha (evento de admissão, renovar, depreciar/invalidar).

- **Só aparece em documento por competência** (`EmployeeDocument.isByCompetency`). É o único capaz de ter mais de uma unidade cobrindo ao mesmo tempo — uma por período — e portanto o único onde pode faltar a de um período específico. Nos demais duas unidades não podem cobrir juntas, e a próxima nasce de Renovar ou de depreciar/invalidar a vigente. O servidor recusa igual (`PMD.DOC28`), então esconder o botão é conveniência, não a regra.
- **A granularidade vem do servidor no detalhe do documento**: `periodTypeId` (1–4, ids do `PeriodType`), `null` = sem competência. `usePreviousPeriod` **não** responde essa pergunta — é falso tanto no documento sem competência quanto no que usa a corrente. Como só o `GET` de detalhe manda o campo, o botão só existe depois de a unidade carregar (é onde ele mora, então não há janela visível).
- **O cliente NÃO calcula a competência.** O diálogo pede a data e diz a granularidade; quem resolve em qual competência ela cai é o servidor. Semana do ano depende de `CultureInfo` e o deslocamento retroativo é regra do template — reimplementar aqui seria uma segunda versão da mesma regra, livre para discordar. A linha recarregada mostra a competência que o servidor decidiu.
- **A verificação de competência ocupada é do servidor, sempre.** A tela tem uma página de unidades; a regra olha todas. `PMD.DOC27` recusa quando já existe unidade naquela competência em **qualquer** estado que não seja Inválido ou Obsoleto — pendente e vencida inclusive —, e a mensagem dele é que manda depreciar ou invalidar antes. Por isso o ViewModel passa o erro por `extractServerMessages`: mensagem fixa aqui deixaria o RH sem saber o que fazer.
- **A unidade nasce pronta**: o servidor grava data, validade e snapshot na criação (mesmo caminho do "editar data"), então não é preciso reabrir a unidade para digitar a mesma data de novo. Efeito colateral esperado: em template com carga horária, a criação pode ser recusada por dia não útil (`PMD.DOC19`) ou conflito de horas (`PMD.DOC10`).
- **`_AddUnitDialog` é `StatefulWidget` e possui o próprio controller** — mesma armadilha do `_ScheduleSignDialog`: o recarregamento que segue a criação reconstrói a árvore enquanto a rota ainda está saindo, e um controller descartado pelo caller estoura ali.
- **Aparece também com a lista vazia**: filtro sem resultado não é motivo para esconder a criação, e é justamente quando falta unidade que ela é procurada.
- **Escopo `document`/`create`**, o mesmo do `POST /document` e do Renovar — sem escopo novo no Keycloak.
...
- **`period` = competência.** `PeriodRule` carries a `PeriodGranularity` (daily/weekly/monthly/yearly, ids matching the backend `PeriodType`: 1–4) and `usePreviousPeriod`. The 4 granularities are hardcoded in the `PeriodGranularity` enum with PT labels — the ids are the contract, the labels are presentation, so no network round-trip for four stable values. The enum lives in `domain/entities/period.dart` (it describes a competência, not a template rule) and `document_template.dart` re-exports it, so importers of either file see it. The form's Regras section has a third switch (`_PeriodRuleTile`) revealing a granularity dropdown + a retroactive switch.
...
- **A versão do Flutter é pinada (`ghcr.io/cirruslabs/flutter:3.38.7`), e é uma de CINCO cópias
  do mesmo número** — as outras são o `FLUTTER_VERSION` de `client-flutter.yml` e de
  `deploy-rufino-android.yml`, o `flutter:` do `codemagic.yaml` (iOS) e o `sdks:` do
  `pubspec.lock`. Onde a versão ficava solta (`stable` no Docker e no Codemagic) o canal andava
  sozinho e o build quebrava sem ninguém ter tocado no repositório: em 3.44 o `IconData` do
