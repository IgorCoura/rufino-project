...
- **A regra de habilitação mora na entidade**, não no widget — `DocumentUnit.canBeDeprecated` (só `OK`), `canBeInvalidated` (`Pending`, `OK` ou `NotApplicable`), `canBeMarkedNotApplicable` (só `Pending`). Depreciada e vencida **nunca** são invalidáveis: são a prova do período coberto, e a API recusa (`PMD.DOC24`).
- **Invalidar é a única saída de uma unidade `NotApplicable`** — e a tela do invalidar muda de texto nesse caso ("Voltar a exigir documento", não "Invalidar documento"): não há erro a desfazer, o documento simplesmente voltou a ser exigido, e a pendente substituta do servidor devolve ao RH o que preencher. Sem isso o documento ficava **sem nenhuma ação possível**, já que criar unidade avulsa saiu da tela. Dispensar é decisão administrativa, não prova de cobertura — por isso é invalidável e depreciada/vencida não são.
- **Depreciar e invalidar ficam FORA do bloco `if (unit.isPending)`** — valem para a unidade em vigência, que é o caso mais comum de ambas, e é o que faz o invalidar aparecer também na `NotApplicable`. Scopes: `deprecate` (novo) e `reject`.
