...
enum PermissionStatus {
  ...
  /// Permissions are available (possibly stale, see `lastError`).
  loaded,

  /// The load failed and nothing is cached.
  error,
}

/// Holds the current user's Keycloak permissions for **one resource server**
/// and exposes query methods.
///
/// Permissions are fetched after login via [loadPermissions] and cached in
/// memory (and persisted through the [PermissionRepository]). Widgets use
/// [hasPermission] or [hasAnyScope] — typically through `PermissionGuard` /
/// `ModuleGuard` — to decide what to render.
///
/// One notifier per audience. A second resource server gets a **subclass**,
/// so `provider` can tell the two apart by type and a resource name granted
/// on one audience can never satisfy a guard written against the other.
///
/// Also observes the app lifecycle — when the app resumes from background,
/// permissions are automatically reloaded (throttled to once every 30
/// seconds).
class PermissionNotifier extends ChangeNotifier with WidgetsBindingObserver {
  /// Creates the notifier over [permissionRepository].
  ...
  /// All permissions currently granted to the user on this audience.
  ...
  bool hasPermission(String resource, String scope) {
    return _permissions.any((p) => p.resource == resource && p.hasScope(scope));
  }
  ...
  Future<void> loadPermissions() async {
    ...
    try {
      ...
      if (_permissions.isEmpty) {
        final cached = await _permissionRepository.getCachedPermissions();
  ...
  ///
  /// Call this on logout so stale permissions are not visible to the next
  /// user.
