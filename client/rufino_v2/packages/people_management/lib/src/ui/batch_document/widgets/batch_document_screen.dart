...
class _BatchDocumentScreenState extends State<BatchDocumentScreen> {
  ...
  Future<void> _pickFileForUnit(BatchDocumentUnitItem unit) async {
    final files = await context.read<FilePickerService>().pickFiles();
  ...
  Future<void> _pickBulkFiles() async {
    ...
    await widget.viewModel.processBulkFiles(files);
    if (!mounted) return;
  ...
  Future<void> _generatePdf() async {
    ...
    final result = await picker.chooseSavePath(
      dialogTitle: 'Salvar PDFs gerados',
      fileName: 'documents.zip',
    );
    if (result != null) {
      await picker.writeToPath(path: result, bytes: bytes);
      if (mounted) {
        ScaffoldMessenger.of(context)
          ..hideCurrentSnackBar()
          ..showSnackBar(
